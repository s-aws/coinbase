...
class FollowUpMaterializationState(str, Enum):
    """Append-only boundaries for one operator materialization attempt.

    ``KNOWN_NOT_INVOKED`` is durable proof that no exchange mutation boundary
    has been crossed.  Every ``*_INVOCATION_STARTED`` state is written before
    the corresponding one-use call and therefore consumes that call boundary
    if its result cannot be classified.
    """

    KNOWN_NOT_INVOKED = "KNOWN_NOT_INVOKED"
    CREATE_INVOCATION_STARTED = "CREATE_INVOCATION_STARTED"
    CREATE_EXPLICITLY_REJECTED = "CREATE_EXPLICITLY_REJECTED"
    CREATE_ACCEPTED_NONTERMINAL = "CREATE_ACCEPTED_NONTERMINAL"
    CREATE_ACCEPTED_TERMINAL = "CREATE_ACCEPTED_TERMINAL"
    CREATE_UNKNOWN_CONSUMED = "CREATE_UNKNOWN_CONSUMED"
    CANCEL_INVOCATION_STARTED = "CANCEL_INVOCATION_STARTED"
    CANCEL_NOT_REQUIRED_TERMINAL = "CANCEL_NOT_REQUIRED_TERMINAL"
    CANCEL_EXPLICITLY_REJECTED = "CANCEL_EXPLICITLY_REJECTED"
...
class FollowUpMaterializedChildTransitionKind(str, Enum):
    """Durable local projection classifications for one claimed child."""

    CREATE_EXPLICITLY_REJECTED = "CREATE_EXPLICITLY_REJECTED"
    CREATE_ACCEPTED_ACTIVE = "CREATE_ACCEPTED_ACTIVE"
    CREATE_ACCEPTED_TERMINAL = "CREATE_ACCEPTED_TERMINAL"
    CREATE_UNKNOWN_QUARANTINED = "CREATE_UNKNOWN_QUARANTINED"
    RECONCILED_ACTIVE = "RECONCILED_ACTIVE"
    RECONCILED_TERMINAL = "RECONCILED_TERMINAL"
    CANCEL_EXPLICITLY_REJECTED_ACTIVE = "CANCEL_EXPLICITLY_REJECTED_ACTIVE"
    CANCEL_ACCEPTED_TERMINAL = "CANCEL_ACCEPTED_TERMINAL"
    CANCEL_UNKNOWN_QUARANTINED = "CANCEL_UNKNOWN_QUARANTINED"
    TERMINAL_WITHOUT_CANCEL = "TERMINAL_WITHOUT_CANCEL"

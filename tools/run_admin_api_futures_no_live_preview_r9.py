"""Prepare the fixed Slice 2R9 Preview producer with live authority disabled.

Preflight validates only the immutable opaque predecessor and a disposable
claim in memory.  It never hydrates credentials, constructs a Coinbase
client, reserves R9, or performs a network call.  The confirmation path stays
hard-disabled until a separate final audit binding activates both gates.
"""
...
from application.admin_api.futures_order_preview import (  # noqa: E402
    ...
    _validate_r9_claim_record,
    _validate_r9_ephemeral_claim_record,
...
from tools import run_admin_api_futures_no_live_preview_r8 as r8_tool  # noqa: E402
from tools import run_admin_api_futures_no_live_preview as base_tool  # noqa: E402


FuturesPreviewOnlyRestClient = r8_tool.FuturesPreviewOnlyRestClient
_suppress_coinbase_sdk_logging = r8_tool._suppress_coinbase_sdk_logging

# R8 is terminally consumed.  R9 remains preparation-only until these two
# independent values are changed by the final audited binding.
R8_PREVIEW_CALL_AUTHORITY_ACTIVE = False
R9_PREVIEW_CALL_AUTHORITY_ACTIVE = False
R9_FINAL_AUDIT_BINDING_READY = False

_R9_EXACT_READ_COUNTERS = {
...
class _R9CanonicalPreviewSession:
    ...
    def __post_init__(self) -> None:
        if self.delegate is None:
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 canonical session is invalid"
            )
    ...
    def __repr__(self) -> str:
        return "<_R9CanonicalPreviewSession private>"
...
class R9AcceptedSessionHandoff:
    """One-use in-memory release of the exact accepted R9 session."""

    delegate: object = field(repr=False)
    account_binding: Slice3CoinbaseAccountBinding = field(repr=False)

    def __repr__(self) -> str:
        return "<R9AcceptedSessionHandoff validated same-session binding>"


def _build_r9_canonical_preview_session(
    *,
    run_secret_lookup: Callable[[str, str | None], str] | None = None,
) -> _R9CanonicalPreviewSession:
    """Build exactly one canonical Default client and R9 Preview facade."""
    ...
    return _R9CanonicalPreviewSession(
        base_tool._build_canonical_default_rest_client(**kwargs)
    )


class DeferredR9PreviewRestClient:
    ...
    def __init__(
        ...
        *,
        ...
        session_factory: Callable[[], _R9CanonicalPreviewSession] | None = None,
        prepared_session: _R9CanonicalPreviewSession | None = None,
    ) -> None:
        if not isinstance(store, FuturesOrderPreviewArtifactStore):
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 artifact store is invalid"
            )
        if prepared_session is not None and session_factory is not None:
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 canonical session source is invalid"
            )
        if prepared_session is not None:
            if type(prepared_session) is not _R9CanonicalPreviewSession:
                raise FuturesOrderPreviewArtifactError(
                    "futures Preview R9 canonical session source is invalid"
                )
            prepared_session.validate()
            factory: Callable[[], _R9CanonicalPreviewSession] | None = None
        else:
            factory = session_factory or _build_r9_canonical_preview_session
        if factory is not None and not callable(factory):
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 canonical session factory is invalid"
            )
        self.__store = store
        ...
        self.__call_attempts = {name: 0 for name in _R9_DEFERRED_CALLS}
    ...
    def _get(self) -> FuturesPreviewOnlyRestClient:
        if not self.__claim_asserted:
            self._assert_r9_claimed()
            self.__claim_asserted = True
        if self.__session is None:
            if self.__hydration_attempted:
                raise FuturesOrderPreviewArtifactError(
                    "futures Preview R9 canonical session is unavailable"
                )
            ...
            with _suppress_coinbase_sdk_logging():
                session = factory()
            if not isinstance(session, _R9CanonicalPreviewSession):
                raise FuturesOrderPreviewArtifactError(
                    "futures Preview R9 canonical session is invalid"
                )
            session.validate()
    ...
    def _assert_r9_claimed(self) -> None:
        try:
            ...
            _validate_r9_claim_record(claim)
        except Exception:
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 claim is unavailable"
            ) from None

    def _consume_call(self, name: str) -> None:
        if name not in self.__call_attempts or self.__call_attempts[name] != 0:
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 call was already attempted"
            )
    ...
    ) -> R9AcceptedSessionHandoff:
        """Release the already-used delegate once after exact R9 acceptance."""

        if self.__accepted_session_consumed:
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 accepted session was already released"
            )
        self.__accepted_session_consumed = True
        session = self.__session
        if session is None:
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 accepted session is unavailable"
            )
        try:
            session.validate()
            ...
            raise FuturesOrderPreviewArtifactError(
                "futures Preview R9 accepted session binding is invalid"
            ) from None
        return R9AcceptedSessionHandoff(
...
def _validate_exact_accepted_pair(
    *,
    ...
    if (
        ephemeral.get("artifact_type") != FUTURES_PREVIEW_R9_ARTIFACT_TYPE
        or ephemeral.get("status") != "accepted"
        or ephemeral.get("outcome") != "accepted"
        or persisted.get("artifact_type") != FUTURES_PREVIEW_R9_ARTIFACT_TYPE
        or persisted.get("status") != "accepted"
        or persisted.get("outcome") != "accepted"
        or dict(stored_terminal) != dict(persisted)
        or ephemeral.get("read_counters") != _R9_EXACT_READ_COUNTERS
        or persisted.get("read_counters") != _R9_EXACT_READ_COUNTERS
        or ephemeral.get("attempt_counters") != _R9_EXACT_ATTEMPT_COUNTERS
        or persisted.get("attempt_counters") != _R9_EXACT_ATTEMPT_COUNTERS
        or dict(call_attempts) != {name: 1 for name in _R9_DEFERRED_CALLS}
...
def production_artifact_path() -> Path:
    """Return the fixed R9 one-use path; configuration cannot redirect it."""

    return FUTURES_PREVIEW_R9_ARTIFACT_PATH


def validate_production_predecessor() -> dict[str, object]:
    """Opaque-hash/stat-bind consumed R8 and all immutable predecessors."""

    return validate_production_futures_order_preview_r8_opaque_chain()


def build_rest_client() -> FuturesPreviewOnlyRestClient:
    """Reuse the fixed no-retry, no-redirect Preview-only facade."""

    return r8_tool.build_rest_client()


def build_r9_store(
    path: Path | None = None,
) -> FuturesOrderPreviewArtifactStore:
    """Build the exact one-use R9 store without reserving its path."""

    return FuturesOrderPreviewArtifactStore(
        path or FUTURES_PREVIEW_R9_ARTIFACT_PATH,
        reservation_lock_nonblocking=True,
    )


def build_r9_producer(
    *,
    rest_client: object,
    store: FuturesOrderPreviewArtifactStore,
    ...
    """Compose R9 with the same accepted-callback contract as R8."""

    return FuturesOrderPreviewProducer(
        rest_client=rest_client,
        store=store,
        predecessor_binding=dict(FUTURES_PREVIEW_R8_TERMINAL_BINDING),
        predecessor_validator=validate_production_predecessor,
        artifact_type=FUTURES_PREVIEW_R9_ARTIFACT_TYPE,
        now=now,
...
_build_r9_store = build_r9_store
_build_r9_producer = build_r9_producer


def build_parser() -> argparse.ArgumentParser:
    """Build the option-minimal R9 preparation CLI."""

    parser = argparse.ArgumentParser(
        description=(
            "Prepare one fixed Default-profile AVAX Futures R9 Preview. "
    ...
    mode.add_argument(
        "--preflight",
        action="store_true",
        help="Validate R9 preparation without credentials or Coinbase access.",
    )
    mode.add_argument(
        "--confirm-one-r9-preview",
        action="store_true",
        help="Consume R9 only after a separate final audited gate activation.",
    )
    return parser
...
def _fixed_blocked_summary(blocker: str) -> dict[str, object]:
    return _summary_before_attempt(
        status="blocked",
        blocker=blocker,
        path=FUTURES_PREVIEW_R9_ARTIFACT_PATH,
        artifact_created=False,
    )


def _validate_fresh_claim_contract(path: Path) -> None:
    """Validate a disposable R9 claim in memory without reserving it."""
    ...
    _validate_r9_ephemeral_claim_record(producer.build_claim())


def main(argv: Sequence[str] | None = None) -> int:
    """Preflight R9 offline or fail closed before any live-capable setup."""

    args = build_parser().parse_args(argv)
    if args.confirm_one_r9_preview and not R9_PREVIEW_CALL_AUTHORITY_ACTIVE:
        print(
            json.dumps(
                _fixed_blocked_summary(
                    "futures_preview_r9_call_authority_inactive"
                ),
                sort_keys=True,
            ),
            file=sys.stderr,
        ...
        return 2
    if args.confirm_one_r9_preview and not R9_FINAL_AUDIT_BINDING_READY:
        print(
            json.dumps(
                _fixed_blocked_summary(
                    "futures_preview_r9_final_audit_binding_incomplete"
                ),
                sort_keys=True,
            ),
            file=sys.stderr,
    ...
    if args.preflight:
        try:
            ...
            if predecessor_binding != FUTURES_PREVIEW_R8_TERMINAL_BINDING:
                raise FuturesOrderPreviewArtifactError(
                    "futures Preview R8 terminal binding changed"
                )
            ...
            print(
                json.dumps(
                    _fixed_blocked_summary(
                        "futures_preview_r9_preflight_validation_blocked"
                    ),
                    sort_keys=True,
                )
            )
        ...
        summary.update(
            {
                ...
                "live_authority_active": R9_PREVIEW_CALL_AUTHORITY_ACTIVE,
                "final_audit_binding_ready": R9_FINAL_AUDIT_BINDING_READY,
            }
        )
    ...
    store = build_r9_store(path)
    deferred_client = DeferredR9PreviewRestClient(store=store)
    producer = build_r9_producer(

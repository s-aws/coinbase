"""Permanent tombstone for consumed R10 and the Slice 3 path that did not run.

The production readiness bit is permanently false and every audited binding is
cleared.  Historical preflight logic remains credential-free and mutation-free
for local regression evidence only.  No callback, audit, or confirmation may
reactivate R10 or bridge its non-accepted terminal into Slice 3.
"""
...
# R10 is terminally consumed.  This bit is permanently hard-false; no audit
# binding may reactivate the generation or its conditional Slice 3 bridge.
R10_SLICE3_END_TO_END_READY = False

# Cleared historical bindings are part of the terminal tombstone.  Any attempt
# to populate them still fails behind the permanently false readiness gate.
...
AUDITED_COMPONENT_SHA256: Mapping[str, str | None] = MappingProxyType(
    {
        ...
        "r10_preview_tool": None,
        "r9_preview_tool": None,
        "r9_end_to_end_tombstone": None,
...
def execute_confirmed_workflow(
    *,
    ...
    """Retain the historical workflow behind a permanent preflight rejection."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect the permanent R10/Slice 3 tombstone. R10 is consumed, "
            "Slice 3 did not run, and confirmation always rejects."
        )
    )

"""Dormant composition root for one R9 Preview and conditional Slice 3.
...
The accepted callback is the sole future bridge from the persisted R9
terminal to Slice 3.  It must reuse the already-hydrated R9 session and is
...
_STAGE0_RUNNER_PATH = (
    _STAGE0_REPO_ROOT / "tools" / "run_admin_api_futures_r9_slice3_end_to_end.py"
)
_STAGE0_PYCACHE_PREFIX = Path("/tmp/coinbase-r9-slice3-empty-pycache")
...
EXPECTED_DEPENDENCY_BINDING_SHA256 = (
    "709a19d5124836365d5ce84b595b5f6a29df947f310aade63aa47386f81fa5c0"
)
...
def _stage0_audit_literal(
    ...
    *,
    ...
    if not isinstance(node, ast.Constant):
        _stage0_raise("futures_r9_slice3_stage0_binding_invalid")
    value = node.value
    if boolean:
...
def _stage0_parse_audit_bindings(payload: bytes) -> dict[str, object]:
    ...
    if (
        ...
        or ready.targets[0].id != "R9_SLICE3_END_TO_END_READY"
    ):
        _stage0_raise("futures_r9_slice3_stage0_binding_invalid")
...
def _stage0_git_text(*args: str) -> str:
    try:
        ...
        _stage0_raise("futures_r9_slice3_stage0_git_binding_invalid")
    ...
    try:
        completed = subprocess.run(
            [
                *_CLOSED_GIT_COMMAND_PREFIX,
                "show",
                f"{revision}:tools/run_admin_api_futures_r9_slice3_end_to_end.py",
            ],
            cwd=_STAGE0_REPO_ROOT,
            env=_CLOSED_GIT_ENV,
            check=True,
...
def _stage0_validate_cache_prefix() -> None:
    try:
        ...
        if (
            ...
            _stage0_raise("futures_r9_slice3_stage0_pycache_invalid")
    except _Stage0Error:
        raise
    except Exception:
        _stage0_raise("futures_r9_slice3_stage0_pycache_invalid")
...
def _stage0_validate_runtime() -> None:
    ...
    if (
        ...
        not in (
            ["--preflight"],
            ["--confirm-one-r9-preview-and-slice3"],
        )
        or sys.path
        != [
        ...
        _stage0_raise("futures_r9_slice3_stage0_isolation_required")
    interpreter = _stage0_read_regular(
        Path(_STAGE0_PYTHON),
        maximum_bytes=_STAGE0_MAX_FILE_BYTES,
        reason="futures_r9_slice3_stage0_interpreter_invalid",
    )
    ...
    if (
        ...
        _stage0_raise("futures_r9_slice3_stage0_interpreter_invalid")
    ...
    try:
        ...
        for path in entries:
            ...
            if metadata.st_uid != os.geteuid():
                _stage0_raise("futures_r9_slice3_aws_cli_binding_invalid")
            if stat.S_ISLNK(metadata.st_mode):
                ...
                payload = _stage0_read_regular(
                    ...
                    reason="futures_r9_slice3_aws_cli_binding_invalid",
                    allow_empty=True,
                )
...
def _stage0_validate_aws_cli() -> None:
    ...
    try:
        ...
        if version_output != _AWS_CLI_VERSION_OUTPUT:
            _stage0_raise("futures_r9_slice3_aws_cli_version_invalid")
    except _Stage0Error:
        raise
    except Exception:
        _stage0_raise("futures_r9_slice3_aws_cli_binding_invalid")
...
def _stage0_secure_credential_file_present(path: Path) -> bool:
    try:
        ...
        _stage0_raise("futures_r9_slice3_credential_provider_invalid")
    if (
        stat.S_ISLNK(metadata.st_mode)
...
def _stage0_validate_credential_provider_presence() -> None:
    ...
    if forbidden_overrides:
        _stage0_raise("futures_r9_slice3_credential_provider_override_present")
    credentials_file = _stage0_secure_credential_file_present(
        Path("/home/developer/.aws/credentials")
    )
    if not credentials_file:
        _stage0_raise("futures_r9_slice3_credential_provider_absent")
...
def _stage0_distribution_target(site_root: Path, record_path: str) -> Path:
    ...
    try:
        if (
            ...
            _stage0_raise("futures_r9_slice3_dependency_record_invalid")
...
def _stage0_verify_distribution(
    ...
    record_payload = _stage0_read_regular(
        ...
        reason="futures_r9_slice3_dependency_record_invalid",
    )
    ...
    if f"Name: {name}" not in metadata_lines or f"Version: {version}" not in (
        ...
        _stage0_raise("futures_r9_slice3_dependency_version_invalid")
...
def _stage0_validate_installed_dependencies() -> str:
    ...
    binding = {
        "schema_version": "r9-slice3-dependency-binding-v1",
        "packages": packages,
    }
    digest = hashlib.sha256(
        json.dumps(
    ...
    if digest != EXPECTED_DEPENDENCY_BINDING_SHA256:
        _stage0_raise("futures_r9_slice3_dependency_binding_invalid")
    return digest
    ...
    def find_spec(cls, fullname, path=None, target=None):
        ...
        if origin in {None, "built-in", "frozen"}:
            ...
            if any(
                ...
                raise ImportError(
                    "futures_r9_slice3_unverified_dependency_import_blocked"
                )
            return spec
...
def _stage0_validate_ready_source(
    ...
    if Path(os.path.abspath(__file__)) != _STAGE0_RUNNER_PATH:
        _stage0_raise("futures_r9_slice3_stage0_runner_path_invalid")
    if _stage0_git_text("status", "--porcelain", "--untracked-files=all"):
        _stage0_raise("futures_r9_slice3_stage0_worktree_not_clean")
    ...
    if (
        ...
        _stage0_raise("futures_r9_slice3_stage0_binding_incomplete")
    ...
    if (
        ...
        _stage0_raise("futures_r9_slice3_stage0_audit_transition_invalid")
    changed = tuple(
        line
    ...
    if changed != ("tools/run_admin_api_futures_r9_slice3_end_to_end.py",):
        _stage0_raise("futures_r9_slice3_stage0_audit_transition_invalid")
    ...
    if (
        ...
        _stage0_raise("futures_r9_slice3_stage0_runner_hash_invalid")
    openapi_payload = _stage0_read_regular(
        _STAGE0_REPO_ROOT / "openapi" / "coinbase-admin-api.yaml",
        maximum_bytes=_STAGE0_MAX_FILE_BYTES,
        reason="futures_r9_slice3_stage0_file_binding_invalid",
    )
    if hashlib.sha256(openapi_payload).hexdigest() != openapi:
        _stage0_raise("futures_r9_slice3_stage0_file_binding_invalid")
    ...
    authorization = _stage0_read_regular(
        ...
        reason="futures_r9_slice3_stage0_authorization_invalid",
    )
    if hashlib.sha256(authorization).hexdigest() != _STAGE0_AUTHORIZATION_SHA256:
        _stage0_raise("futures_r9_slice3_stage0_authorization_invalid")
...
def _stage0_before_project_imports() -> tuple[bool, object | None]:
    ...
    try:
        runner_payload = _stage0_read_regular(
            source_path,
            maximum_bytes=_STAGE0_MAX_FILE_BYTES,
            reason="futures_r9_slice3_stage0_runner_path_invalid",
        )
        ...
        if __name__ != "__main__":
            _stage0_raise("futures_r9_slice3_stage0_direct_import_blocked")
        ...
        for site_root in dict.fromkeys(
            ...
            if site_root in sys.path:
                _stage0_raise("futures_r9_slice3_stage0_import_path_invalid")
...
from application.admin_api.futures_terminal_roundtrip_activation import (  # noqa: E402
    ...
    Slice3AcceptedR8Binding as Slice3AcceptedR9Binding,
...
RUNNER_PATH = REPO_ROOT / "tools" / "run_admin_api_futures_r9_slice3_end_to_end.py"
...
class R9Slice3RunnerError(RuntimeError):
    """A fixed, sanitized fail-closed reason from the composition root."""


def _validate_r9_documented_preview_expiry_boundary(
    *,
    ephemeral_evidence: Mapping[str, Any],
    now: datetime,
) -> None:
    """Require authoritative expiry before any accepted-R9 plan binding."""
    ...
    if (
        ...
        _raise("futures_r9_slice3_preview_expiry_evidence_invalid")
    try:
...
def build_slice3_admitted_plan_from_r9(
    *,
    ephemeral_evidence: Mapping[str, Any],
    ...
    """Adapt the dormant R9 surface to the still-R8-named Slice 3 contract.
    ...
    the generation-specific runner never rewrites an accepted R9 artifact.
    """
    ...
    return _build_slice3_admitted_plan_from_r8(
        ...
        accepted_r8_binding=accepted_r9_binding,
...
def build_slice3_activation_manifest(
    *,
    plan: object,
    persisted_terminal: Mapping[str, Any],
    r9_artifact_file_sha256: str,
    authorization_text: bytes,
    admission_seal: object,
    now: datetime,
) -> object:
    """Keep the R9 runner vocabulary outside the legacy manifest API."""

    return _build_slice3_activation_manifest(
        plan=plan,
        persisted_terminal=persisted_terminal,
        r8_artifact_file_sha256=r9_artifact_file_sha256,
...
def _raise(reason: str) -> None:
    raise R9Slice3RunnerError(reason)
...
def _defer_slice3_termination_signals():
    ...
    if (
        ...
        _raise("futures_r9_slice3_signal_deferral_unavailable")
    try:
    ...
    try:
        ...
        try:
            ...
            _raise("futures_r9_slice3_signal_restore_failed")
...
def _sha256_file(path: Path) -> str:
    return hashlib.sha256(
        _read_regular_file(
            ...
            reason="futures_r9_slice3_file_binding_invalid",
        )
    ).hexdigest()
...
def _audit_literal(
    ...
    *,
    ...
    if not isinstance(node, ast.Constant):
        _raise("futures_r9_slice3_runner_binding_invalid")
    value = node.value
    if boolean:
        if not isinstance(value, bool):
            _raise("futures_r9_slice3_runner_binding_invalid")
...
def _current_runner_bytes() -> bytes:
    ...
    return _read_regular_file(
        RUNNER_PATH,
        maximum_bytes=_MAX_AUDITED_FILE_BYTES,
        reason="futures_r9_slice3_runner_binding_invalid",
    )
...
def _current_backend_revision() -> str:
    ...
    if _REVISION_RE.fullmatch(revision) is None:
        _raise("futures_r9_slice3_backend_revision_unavailable")
    return revision
...
def _runner_bytes_at_revision(revision: str) -> bytes:
    try:
        ...
        _raise("futures_r9_slice3_audit_base_runner_unavailable")
...
def _validate_audit_commit_transition(base_revision: str) -> str:
    ...
    expected_path = "tools/run_admin_api_futures_r9_slice3_end_to_end.py"
    if changed != (expected_path,):
        _raise("futures_r9_slice3_audit_commit_scope_invalid")
    ...
    if base_logic != current_logic:
        _raise("futures_r9_slice3_audit_commit_logic_changed")
    return current
...
def _validate_local_runtime_readiness() -> str:
    ...
    try:
        ...
        if _SAFE_REASON_RE.fullmatch(reason) is None:
            reason = "futures_r9_slice3_local_runtime_invalid"
        _raise(reason)
...
def _validate_audited_state() -> tuple[str, str]:
    if (
        ...
        _raise("futures_r9_slice3_audit_binding_incomplete")
    if _current_runner_logic_sha256() != AUDITED_RUNNER_LOGIC_SHA256:
        _raise("futures_r9_slice3_audited_runner_changed")
    ...
    if openapi_sha256 != AUDITED_OPENAPI_SHA256:
        _raise("futures_r9_slice3_audited_openapi_changed")
    for name, path in AUDITED_COMPONENT_PATHS.items():
        if _sha256_file(path) != AUDITED_COMPONENT_SHA256[name]:
            _raise("futures_r9_slice3_audited_component_changed")
...
def _validate_attempt_paths_absent() -> None:
    if len(FIXED_ATTEMPT_PATHS) != 7 or len(set(FIXED_ATTEMPT_PATHS)) != 7:
        _raise("futures_r9_slice3_attempt_path_binding_invalid")
    for path in FIXED_ATTEMPT_PATHS:
        ...
        try:
            ...
            _raise("futures_r9_slice3_attempt_path_not_fresh")
        _raise("futures_r9_slice3_attempt_path_not_fresh")


def _validate_immutable_predecessors() -> None:
    try:
        binding = dict(r9_tool.validate_production_predecessor())
    except Exception:
        _raise("futures_r9_slice3_predecessor_binding_invalid")
    if binding != FUTURES_PREVIEW_R8_TERMINAL_BINDING:
        _raise("futures_r9_slice3_predecessor_binding_invalid")


def _validate_exclusive_r9_entrypoint() -> None:
    if (
        r9_tool.R8_PREVIEW_CALL_AUTHORITY_ACTIVE is not False
        or r9_tool.R9_PREVIEW_CALL_AUTHORITY_ACTIVE is not False
        or r9_tool.R9_FINAL_AUDIT_BINDING_READY is not False
    ):
        _raise("futures_r9_slice3_parallel_preview_path_enabled")


def _validate_fresh_r9_claim_contract() -> None:
    try:
        r9_tool._validate_fresh_claim_contract(FUTURES_PREVIEW_R9_ARTIFACT_PATH)
    except Exception:
        _raise("futures_r9_slice3_claim_contract_invalid")
...
def validate_production_preflight() -> PreflightEvidence:
    ...
    _validate_exclusive_r9_entrypoint()
    _validate_immutable_predecessors()
    _validate_fresh_r9_claim_contract()
    if not _runtime_enforces_admission_binding():
        _raise("futures_r9_slice3_runtime_admission_binding_unavailable")
...
def _lookup_fixed_r9_secret(secret_id: str, region: str | None) -> str:
    """Resolve the exact Default secret once with a closed AWS CLI process."""

    if secret_id != "coinbase" or region != "us-east-1":
        _raise("futures_r9_slice3_credential_preparation_failed")
    argv = [
...
def _prepare_r9_canonical_session() -> Any:
    """Resolve credentials and construct the zero-retry client before claim."""

    try:
        with r9_tool._suppress_coinbase_sdk_logging():
            session = r9_tool._build_r9_canonical_preview_session(
                run_secret_lookup=_lookup_fixed_r9_secret,
            )
        if type(session) is not r9_tool._R9CanonicalPreviewSession:
            _raise("futures_r9_slice3_credential_preparation_failed")
        session.validate()
        return session
    except R9Slice3RunnerError:
...
def build_deferred_r9_session(
    *,
    store: FuturesOrderPreviewArtifactStore,
    prepared_session: Any,
) -> Any:
    """Inject the exact already-prepared R9 session into its one-use holder."""

    if type(prepared_session) is not r9_tool._R9CanonicalPreviewSession:
        _raise("futures_r9_slice3_credential_preparation_failed")
    prepared_session.validate()
    session = r9_tool.DeferredR9PreviewRestClient(
        store=store,
        prepared_session=prepared_session,
    )
    take = getattr(session, "take_accepted_session", None)
    if not callable(take):
        _raise("futures_r9_slice3_same_session_handoff_unavailable")
    return session


def _build_r9_store() -> FuturesOrderPreviewArtifactStore:
    return r9_tool.build_r9_store(FUTURES_PREVIEW_R9_ARTIFACT_PATH)
...
def _sanitized_handoff_halted_result(
    ...
    *,
    ...
    try:
        ...
        _raise("futures_r9_slice3_accepted_handoff_terminal_invalid")
    expected_reasons = {
...
def recover_r9_accepted_handoff_offline(
    *,
    ...
    if not R9_SLICE3_END_TO_END_READY:
        _raise("futures_r9_slice3_workflow_not_ready")
    ...
    for path in (
        ...
        try:
            ...
            _raise("futures_r9_slice3_offline_handoff_recovery_blocked")
        _raise("futures_r9_slice3_offline_handoff_recovery_blocked")
    try:
...
def _build_r9_producer(
    *,
    deferred_session: Any,
    store: FuturesOrderPreviewArtifactStore,
) -> FuturesOrderPreviewProducer:
    return r9_tool.build_r9_producer(
...
def _validate_accepted_handoff_provenance(
    *,
    ...
    """Bind Slice 3 to the exact callback-local R9 delegate and fixed file."""

    try:
        if (
            type(deferred_session) is not r9_tool.DeferredR9PreviewRestClient
            or type(accepted_session) is not r9_tool.R9AcceptedSessionHandoff
            or type(r9_store) is not FuturesOrderPreviewArtifactStore
            or getattr(
                deferred_session,
                "_DeferredR9PreviewRestClient__store",
                None,
            )
            is not r9_store
            or r9_store.path != FUTURES_PREVIEW_R9_ARTIFACT_PATH
            or _SHA256_RE.fullmatch(r9_artifact_file_sha256) is None
        ):
            raise ValueError
        canonical_session = getattr(
            deferred_session,
            "_DeferredR9PreviewRestClient__session",
            None,
        )
        if type(canonical_session) is not r9_tool._R9CanonicalPreviewSession:
            raise ValueError
        ...
        if (
            ...
            or dict(r9_store.read_completed()) != dict(persisted_terminal)
            or _sha256_file(FUTURES_PREVIEW_R9_ARTIFACT_PATH)
            != r9_artifact_file_sha256
        ):
            raise ValueError
    except Exception:
        _raise("futures_r9_slice3_same_session_handoff_invalid")
...
def _validated_detached_terminal_envelope(
    ...
    *,
    ...
    try:
        ...
        _raise("futures_r9_slice3_terminal_result_invalid")
...
def run_accepted_slice3_handoff(
    *,
    ...
    deferred_session: Any,
    r9_store: FuturesOrderPreviewArtifactStore,
    ...
    try:
        def validate_delegation() -> tuple[str, str, object, object]:
            ...
            if (
                ...
                or not isinstance(now, datetime)
                or now.tzinfo is None
                or not isinstance(r9_artifact_file_sha256, str)
                or _SHA256_RE.fullmatch(r9_artifact_file_sha256) is None
            ):
                _raise("futures_r9_slice3_accepted_handoff_binding_invalid")
            ...
            _validate_accepted_handoff_provenance(
                ...
                r9_store=r9_store,
                persisted_terminal=persisted_terminal,
                r9_artifact_file_sha256=r9_artifact_file_sha256,
            )
        ...
        def build_plan() -> tuple[object, object]:
            ...
            accepted_r9_binding = Slice3AcceptedR9Binding.from_accepted_evidence(
                artifact_file_sha256=r9_artifact_file_sha256,
                evidence=persisted_terminal,
            )
            return build_slice3_admitted_plan_from_r9(
                ephemeral_evidence=ephemeral_evidence,
                persisted_terminal=persisted_terminal,
                accepted_r9_binding=accepted_r9_binding,
                account_binding=account_binding,
        ...
        def seal_admission() -> tuple[object, object]:
            ...
            if (
                ...
                _raise("futures_r9_slice3_admission_seal_invalid")
        ...
        def seal_activation() -> tuple[object, object]:
            ...
            if (
                ...
                _raise("futures_r9_slice3_activation_seal_invalid")
        ...
        def prepare_port_dependencies() -> tuple[object, object, object, str]:
            ...
            if (
                ...
                _raise("futures_r9_slice3_portfolio_binding_invalid")
            return (
                action_store,
                read_journal,
                terminal_store,
        ...
        def delegate_and_sanitize() -> SanitizedSlice3Result:
            ...
            if len(constructed_ports) != 1:
                _raise("futures_r9_slice3_one_port_invariant_failed")
        ...
        _raise("futures_r9_slice3_accepted_handoff_blocked")
...
def execute_confirmed_workflow(
    *,
    ...
    """Consume R9 once and invoke Slice 3 only from its accepted callback."""
    ...
    prepared_session = _prepare_r9_canonical_session()
    _validate_attempt_paths_absent()
    store = _build_r9_store()
    deferred_session = build_deferred_r9_session(
        store=store,
        prepared_session=prepared_session,
    )
    producer = _build_r9_producer(
        deferred_session=deferred_session,
        store=store,
    )
    ...
    def accepted_callback(
        ...
        artifact_sha256 = _sha256_file(FUTURES_PREVIEW_R9_ARTIFACT_PATH)
        r9_evidence_sha256 = persisted.get("evidence_sha256")
        try:
            ...
            handoff_lease = handoff_store.reserve(
                ...
                preview_evidence_sha256=r9_evidence_sha256,
                now=clock(),
            )
        except AcceptedHandoffArtifactError:
            _raise("futures_r9_slice3_accepted_handoff_terminal_invalid")
        ...
        try:
            ...
            result = run_accepted_slice3_handoff(
                ...
                r9_artifact_file_sha256=artifact_sha256,
                now=clock(),
        ...
        if slice3_holder:
            _raise("futures_r9_slice3_callback_reentered")
    ...
    try:
        ...
        try:
            ...
            _raise("futures_r9_terminal_unavailable_after_consumption")
    if not isinstance(terminal, Mapping):
        _raise("futures_r9_terminal_invalid")
    accepted = (
        terminal.get("status") == "accepted" and terminal.get("outcome") == "accepted"
    )
    if accepted and len(slice3_holder) != 1:
        _raise("futures_r9_slice3_accepted_handoff_incomplete")
    if not accepted and slice3_holder:
        _raise("futures_r9_slice3_unaccepted_handoff_invalid")
...
def safe_terminal_summary(result: EndToEndExecutionResult) -> dict[str, object]:
    ...
    if result.dependency_binding_sha256 != EXPECTED_DEPENDENCY_BINDING_SHA256:
        _raise("futures_r9_slice3_dependency_binding_invalid")
    r9_status = _safe_atom(result.terminal.get("status"), status=True)
    r9_outcome = _safe_atom(result.terminal.get("outcome"), status=True)
    summary: dict[str, object] = {
        "status": "terminal_halted",
        "r9_status": r9_status or "unknown",
        "r9_outcome": r9_outcome or "unknown",
        "r9_artifact_created": True,
        ...
        summary.update(
            {
                ...
                "slice3_reason_code": "r9_not_accepted",
...
def _before_attempt_summary(
    *,
    ...
    return {
        "status": status,
        "blocker": blocker,
        "workflow_ready": R9_SLICE3_END_TO_END_READY,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "One fixed R9 Preview and conditional terminal Slice 3; no retry, "
...
def _safe_error_reason(exc: R9Slice3RunnerError) -> str:
    reason = str(exc)
    if _SAFE_REASON_RE.fullmatch(reason) is None or not reason.startswith(
        "futures_r9_slice3_"
    ):
        return "futures_r9_slice3_validation_blocked"
    return reason


def main(argv: Sequence[str] | None = None) -> int:
    args = build_parser().parse_args(argv)
    if args.confirm_one_r9_preview_and_slice3 and not R9_SLICE3_END_TO_END_READY:
        print(
            json.dumps(
                _before_attempt_summary(
                    status="blocked",
                    blocker="futures_r9_slice3_workflow_not_ready",
                ),
                sort_keys=True,
            ),
            file=sys.stderr,
    ...
    try:
    ...
    except R9Slice3RunnerError as exc:
        print(
            json.dumps(
                _before_attempt_summary(
                    status="blocked",
        ...
        print(
            json.dumps(
                _before_attempt_summary(
                    status="blocked",
                    blocker="futures_r9_slice3_validation_blocked",
                ),
                sort_keys=True,
            )
        )
    ...
    try:
        ...
        print(
            json.dumps(
                {
                    "status": "blocked_or_consumed",
                    "blocker": "futures_r9_slice3_outer_baseexception_blocked",
                    "raw_response_included": False,
                    "private_identifier_values_included": False,
                    "withheld_exception_text_included": False,
                },

...
Current M57 phases 6881-6900 add disabled futures request payload validation
...
and `request_payload_validation_record_reduce_only_semantics` remain
backend-owned display evidence. Rows expose `reduce_only_semantics_ref`,
`reduce_only_semantics_contract_ref`, `evidence_routes`,
...
`validation_record_reduce_only_semantics_ready=false` while preserving no
payload validation, no reduce-only semantics acceptance, no live account/risk

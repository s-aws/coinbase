...
class TestAdmissionOpenHooks:
    """Admission-open hooks follow the operator-visible lifecycle."""

    @pytest.mark.regression
    def test_hook_follows_admission_open_and_unregister(self):
        startup = RuntimeController()
        startup_calls = []
        assert startup.register_admission_open_hook(
            "owner", lambda: startup_calls.append(startup.state)
        ) is True

        assert startup.complete_startup() is True
        assert startup_calls == [EngineState.RUNNING]

        paused = RuntimeController()
        paused_calls = []
        assert paused.register_admission_open_hook(
            "owner", lambda: paused_calls.append(paused.state)
        ) is True
        assert paused.request_pause() is True
        assert paused.complete_startup() is True
        assert paused.state is EngineState.PAUSED
        assert paused_calls == []

        assert paused.resume() is True
        assert paused_calls == [EngineState.RUNNING]

        assert paused.unregister_admission_open_hook("owner") is True
        assert paused.request_pause() is True
        assert paused.resume() is True
        assert paused_calls == [EngineState.RUNNING]

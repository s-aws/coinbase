...
from tools import run_admin_api_futures_r10_slice3_end_to_end as runner
...
def _patch_clean_preflight(
    ...
    # aggregate tuple and the imported concrete R10 constant.  Patching only
    # the tuple previously allowed a synthetic failure-path test to reserve
    # the production R10 artifact before its mocked credential probe failed.
    monkeypatch.setattr(
        runner,
    ...
    monkeypatch.setattr(
        ...
        lambda: dict(runner.FUTURES_PREVIEW_R9_TERMINAL_BINDING),
    )
    monkeypatch.setattr(
        runner.r10_tool,
    ...
    monkeypatch.setattr(
        runner.r10_tool,
        "R10_PREVIEW_CALL_AUTHORITY_ACTIVE",
        False,
    )
    monkeypatch.setattr(
        runner.r10_tool,
        "R10_FINAL_AUDIT_BINDING_READY",
        False,
    )
    monkeypatch.setattr(
        runner,
...
def _new_handoff_terminalizer(
    ...
    lease = store.reserve(
        ...
        preview_artifact_type=runner.FUTURES_PREVIEW_R10_ARTIFACT_TYPE,
...
def test_readiness_and_audit_constants_are_hard_fail_closed() -> None:
    assert runner.R10_SLICE3_END_TO_END_READY is False
...
def test_audit_components_bind_r10_r9_chain_and_current_r9_tombstone() -> None:
    assert runner.AUDITED_COMPONENT_PATHS["r10_preview_tool"] == (
        runner.REPO_ROOT / "tools" / "run_admin_api_futures_no_live_preview_r10.py"
    )
    assert runner.AUDITED_COMPONENT_PATHS["r9_preview_tool"] == (
        runner.REPO_ROOT / "tools" / "run_admin_api_futures_no_live_preview_r9.py"
    )
    assert runner.AUDITED_COMPONENT_PATHS["r9_end_to_end_tombstone"] == (
        runner.REPO_ROOT / "tools" / "run_admin_api_futures_r9_slice3_end_to_end.py"
    )
    assert tuple(runner.AUDITED_COMPONENT_SHA256) == tuple(
        runner.AUDITED_COMPONENT_PATHS
    )


def test_r9_end_to_end_entrypoint_is_a_permanent_no_io_tombstone(
...
def test_r10_plan_adapter_checks_documented_expiry_before_legacy_binding(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        runner,
    ...
    with pytest.raises(
        ...
        runner.build_slice3_admitted_plan_from_r10(
            ephemeral_evidence={
...
def test_r10_predecessor_binding_is_exact_immutable_r9_terminal(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    observed = dict(runner.FUTURES_PREVIEW_R9_TERMINAL_BINDING)
    monkeypatch.setattr(
        runner.r10_tool,
        "validate_production_predecessor",
        lambda: observed,
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_predecessor_binding_invalid$",
    ):
        runner._validate_immutable_predecessors()


def test_r10_composition_uses_only_the_r10_producer_api(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    deferred = object()
    store = object()
    ...
    def build_r10_producer(**kwargs):
        calls.append(dict(kwargs))
        return producer

    monkeypatch.setattr(runner.r10_tool, "build_r10_producer", build_r10_producer)
    assert runner._build_r10_producer(
...
def test_stage0_import_guard_rejects_unverified_site_module(
    ...
    with pytest.raises(
        ImportError,
        match="^futures_r10_slice3_unverified_dependency_import_blocked$",
    ):
...
def test_ready_cli_without_isolated_python_blocks_before_project_import(
    ...
    source = runner.RUNNER_PATH.read_text(encoding="utf-8").replace(
        "R10_SLICE3_END_TO_END_READY = False",
    ...
    assert "futures_r10_slice3_stage0_isolation_required" in rendered
...
def test_ready_isolated_cli_rejects_untracked_before_project_import(
    ...
    source = source.replace(
        ...
        'Path("/tmp/coinbase-r10-slice3-empty-pycache")',
    ...
    subprocess.run(
        ["git", "add", "tools/run_admin_api_futures_r10_slice3_end_to_end.py"],
        cwd=repo,
        check=True,
        capture_output=True,
        text=True,
    ...
    rendered = completed.stdout + completed.stderr
    assert completed.returncode == 2
    assert "futures_r10_slice3_stage0_worktree_not_clean" in rendered
    assert "Traceback" not in rendered
    assert "ModuleNotFoundError" not in rendered
...
def test_runner_logic_normalization_rejects_executable_binding_statement() -> None:
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_runner_binding_invalid$",
    ):
...
def _patch_valid_audit_transition(
    ...
    outputs = {
        ...
        (
            ...
            f"{base}..{current}",
            "--",
        ): "tools/run_admin_api_futures_r10_slice3_end_to_end.py",
    }
...
@pytest.mark.parametrize(
    ("mutation", "reason"),
    [
        ("dirty", "futures_r10_slice3_tracked_worktree_not_clean"),
        ("branch", "futures_r10_slice3_branch_not_main"),
        ("origin", "futures_r10_slice3_origin_main_not_synchronized"),
        ("parent", "futures_r10_slice3_audit_commit_transition_invalid"),
        ("scope", "futures_r10_slice3_audit_commit_scope_invalid"),
        ("logic", "futures_r10_slice3_audit_commit_logic_changed"),
    ],
)
def test_audit_transition_rejects_drift_outside_binding_block(
    ...
    with pytest.raises(runner.R10Slice3RunnerError, match=f"^{reason}$"):
...
def test_preflight_validates_every_binding_without_client_or_artifact(
    ...
    monkeypatch.setattr(runner, "R10_SLICE3_END_TO_END_READY", True)
    monkeypatch.setattr(
        runner,
        "build_deferred_r10_session",
...
def test_preflight_still_checks_audit_placeholders_while_gate_is_false(
    ...
    assert summary["blocker"] == "futures_r10_slice3_audit_binding_incomplete"
...
def test_preflight_rejects_runner_logic_drift(
    ...
    assert json.loads(capsys.readouterr().out)["blocker"] == (
        "futures_r10_slice3_audited_runner_changed"
    )
...
def test_preflight_requires_runtime_enforced_admission_binding(
    ...
    assert runner.main(["--preflight"]) == 2
    assert json.loads(capsys.readouterr().out)["blocker"] == (
        "futures_r10_slice3_runtime_admission_binding_unavailable"
    )
...
def test_preflight_rejects_authorization_byte_change_without_leaking_text(
    ...
    assert json.loads(rendered)["blocker"] == (
        "futures_r10_slice3_authorization_binding_invalid"
    )
...
def test_preflight_rejects_local_credential_blocker_before_claim_contract(
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(
            runner.R10Slice3RunnerError(
                "futures_r10_slice3_credential_provider_absent"
            )
        ),
    )
    monkeypatch.setattr(
    ...
    assert runner.main(["--preflight"]) == 2
    summary = json.loads(capsys.readouterr().out)
    assert summary["blocker"] == "futures_r10_slice3_credential_provider_absent"
...
def test_stage0_credential_provider_requires_fixed_file_and_no_overrides(
    ...
    with pytest.raises(
        runner._Stage0Error,
        match="^futures_r10_slice3_credential_provider_override_present$",
    ):
        runner._stage0_validate_credential_provider_presence()
    ...
    with pytest.raises(
        runner._Stage0Error,
        match="^futures_r10_slice3_credential_provider_absent$",
    ):
        runner._stage0_validate_credential_provider_presence()


def test_fixed_r10_secret_lookup_uses_only_the_pinned_bounded_aws_cli(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    calls: list[tuple[list[str], dict[str, object]]] = []
    ...
    assert runner._lookup_fixed_r10_secret("coinbase", "us-east-1") == (
        secret_payload
    )
    assert len(calls) == 1
    args, kwargs = calls[0]
...
def test_fixed_r10_secret_lookup_rejects_credential_scope_drift(
    secret_id: str,
    region: str | None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_credential_preparation_failed$",
    ):
        runner._lookup_fixed_r10_secret(secret_id, region)


def test_fixed_r10_secret_lookup_withholds_cli_failure_text(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        runner.subprocess,
    ...
    with pytest.raises(
        ...
        runner._lookup_fixed_r10_secret("coinbase", "us-east-1")
...
def test_confirmed_credential_preparation_failure_leaves_every_path_absent(
    ...
    def fail_construction(*, run_secret_lookup):
        ...
        assert run_secret_lookup is runner._lookup_fixed_r10_secret
...
def test_malformed_coinbase_key_fails_local_probe_before_claim_or_network(
    ...
    production_path = (
        runner.FUTURES_PREVIEW_R10_ARTIFACT_PATH
    )
    production_metadata = (
        tuple(
    ...
    _authorization, paths = _patch_clean_preflight(monkeypatch, tmp_path)
    assert runner.FUTURES_PREVIEW_R10_ARTIFACT_PATH == paths[0]
    assert runner._build_r10_store().path == paths[0]
    monkeypatch.setattr(runner, "R10_SLICE3_END_TO_END_READY", True)
    monkeypatch.setattr(
        runner,
        "_lookup_fixed_r10_secret",
...
def test_preflight_rejects_every_parallel_preview_entrypoint(
    ...
    _patch_clean_preflight(monkeypatch, tmp_path)
    monkeypatch.setattr(runner, "R10_SLICE3_END_TO_END_READY", True)
    monkeypatch.setattr(runner.r10_tool, drifted_constant, True)

    assert runner.main(["--preflight"]) == 2
    assert json.loads(capsys.readouterr().out)["blocker"] == (
        "futures_r10_slice3_parallel_preview_path_enabled"
    )
...
def test_preflight_rejects_every_consumed_fixed_path(
    ...
    assert runner.main(["--preflight"]) == 2
    summary = json.loads(capsys.readouterr().out)
    assert summary["blocker"] == "futures_r10_slice3_attempt_path_not_fresh"
...
def test_confirmation_gate_blocks_before_preflight_or_client(
    ...
    monkeypatch.setattr(runner, "R10_SLICE3_END_TO_END_READY", False)
    monkeypatch.setattr(
        runner,
    ...
    assert runner.main(["--confirm-one-r10-preview-and-slice3"]) == 2
    summary = json.loads(capsys.readouterr().err)
...
def test_callable_execution_runs_preflight_before_store_or_client(
    ...
    monkeypatch.setattr(
        ...
        "validate_production_preflight",
        lambda: (_ for _ in ()).throw(
            runner.R10Slice3RunnerError("futures_r10_slice3_audit_binding_incomplete")
        ),
    )
    monkeypatch.setattr(
        runner,
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_audit_binding_incomplete$",
    ):
...
def test_callable_execution_rejects_wrong_authorization_before_store(
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_authorization_binding_invalid$",
    ):
...
def test_cli_sanitizes_unexpected_exception_without_traceback(
    ...
    if phase == "preflight":
        ...
        argv = ["--confirm-one-r10-preview-and-slice3"]
    ...
    assert summary["blocker"] == "futures_r10_slice3_validation_blocked"
...
def test_production_composition_fails_closed_without_runtime_admission_binding(
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_runtime_admission_binding_unavailable$",
    ):
        runner.run_accepted_slice3_handoff(
            ...
            r10_store=SimpleNamespace(),
            callback_capability=object(),
            authorization_bytes=b"authorized",
            r10_artifact_file_sha256=_SHA_A,
            now=_NOW,
...
def test_accepted_handoff_rejects_unowned_session_provenance(
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_same_session_handoff_invalid$",
    ):
        runner.run_accepted_slice3_handoff(
...
def test_accepted_handoff_seals_admission_then_activation_and_runs_one_port(
    ...
    class R10BindingFactory:
        ...
        def from_accepted_evidence(cls, **kwargs):
            ...
            events.append("r10_binding")
            return accepted_r10_binding

    def admitted_plan_builder(**kwargs):
        assert kwargs["accepted_r10_binding"] is accepted_r10_binding
    ...
    monkeypatch.setattr(runner, "Slice3AcceptedR10Binding", R10BindingFactory)
    monkeypatch.setattr(
        runner,
        "build_slice3_admitted_plan_from_r10",
...
def test_confirmed_workflow_passes_exact_accepted_session_to_handoff_once(
    ...
    monkeypatch.setattr(runner, "_prepare_r10_canonical_session", prepare_session)
    monkeypatch.setattr(runner, "_build_r10_store", lambda: Store())
    monkeypatch.setattr(
        runner,
        "_build_r10_producer",
    ...
    monkeypatch.setattr(
        ...
        lambda path: (
            _SHA_B
            if path == runner.FUTURES_PREVIEW_R10_ARTIFACT_PATH
    ...
    assert handoff_calls[0]["r10_store"].path == runner.FIXED_ATTEMPT_PATHS[0]
    assert handoff_calls[0]["authorization_bytes"] == authorization_bytes
    assert handoff_calls[0]["r10_artifact_file_sha256"] == _SHA_B
...
def test_missing_preview_expiry_halts_at_plan_boundary_and_reports_terminal(
    ...
    persisted = {
        "status": "accepted",
        "outcome": "accepted",
        "artifact_type": runner.FUTURES_PREVIEW_R10_ARTIFACT_TYPE,
        "evidence_sha256": _SHA_A,
    }
    ...
    monkeypatch.setattr(runner, "_prepare_r10_canonical_session", object)
    monkeypatch.setattr(
        runner,
        "build_deferred_r10_session",
        lambda **_kwargs: Deferred(),
    )
    monkeypatch.setattr(runner, "_build_r10_store", Store)
    monkeypatch.setattr(
        runner,
        "_build_r10_producer",
    ...
    def missing_expiry_handoff(**kwargs):
        def fail_plan():
            raise runner.R10Slice3RunnerError(
...
def test_real_r10_handoff_terminalizes_at_documented_expiry_plan_boundary(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    monkeypatch.setattr(
        runner,
        "Slice3AcceptedR10Binding",
        SimpleNamespace(
            from_accepted_evidence=lambda **_kwargs: (_ for _ in ()).throw(
                AssertionError("accepted binding built before documented expiry")
            )
        ),
    )
    monkeypatch.setattr(
        ...
        lambda **_kwargs: (_ for _ in ()).throw(
            AssertionError("admission built before documented expiry")
        ),
    )
    monkeypatch.setattr(
        runner,
        "production_slice3_admission_store",
        lambda: (_ for _ in ()).throw(
            AssertionError("admission store opened before documented expiry")
        ),
    )
    monkeypatch.setattr(
        runner,
        "production_slice3_activation_store",
        lambda: (_ for _ in ()).throw(
            AssertionError("activation store opened before documented expiry")
        ),
    )
    monkeypatch.setattr(
        runner,
        "build_slice3_activation_manifest",
        lambda **_kwargs: (_ for _ in ()).throw(
            AssertionError("activation built before documented expiry")
        ),
    )
    monkeypatch.setattr(
        runner,
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_accepted_handoff_blocked$",
    ):
        runner.run_accepted_slice3_handoff(
            ephemeral_evidence={
                "completed_at": _NOW.isoformat(),
    ...
    assert terminal.preview_generation == 10
    assert terminal.preview_artifact_type == (
        runner.FUTURES_PREVIEW_R10_ARTIFACT_TYPE
    )
...
def test_offline_handoff_recovery_never_constructs_credentials_preview_or_ports(
    ...
    preview_path = tmp_path / "r10.jsonl"
    ...
    monkeypatch.setattr(runner, "FUTURES_PREVIEW_R10_ARTIFACT_PATH", preview_path)
    ...
    monkeypatch.setattr(
        runner,
        "_prepare_r10_canonical_session",
    ...
    result = runner.recover_r10_accepted_handoff_offline(
        now_provider=lambda: _NOW,
    )
...
def test_confirmed_workflow_does_not_take_session_when_r10_is_not_accepted(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _patch_clean_preflight(monkeypatch, tmp_path)
...
def test_safe_summary_never_includes_raw_identifiers_or_exception_text() -> None:
    ...
    assert summary["r10_status"] == "accepted"
...
def test_signal_deferral_rejects_non_main_thread(
    ...
    with pytest.raises(
        runner.R10Slice3RunnerError,
        match="^futures_r10_slice3_signal_deferral_unavailable$",
    ):
        with runner._defer_slice3_termination_signals():
...
def test_outer_cli_baseexception_is_fixed_and_sanitized(
    ...
    assert "Traceback" not in rendered
    assert json.loads(rendered)["blocker"] == (
        "futures_r10_slice3_outer_baseexception_blocked"
    )
...
def test_cli_exit_matches_slice3_terminal_state(
    ...
    assert runner.main(["--confirm-one-r10-preview-and-slice3"]) == expected_exit
    summary = json.loads(capsys.readouterr().out)
    assert summary["status"] == expected_status


def test_r10_nonacceptance_is_terminal_no_mutation_success(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    monkeypatch.setattr(runner, "R10_SLICE3_END_TO_END_READY", True)
    ...
    assert runner.main(["--confirm-one-r10-preview-and-slice3"]) == 0
    summary = json.loads(capsys.readouterr().out)

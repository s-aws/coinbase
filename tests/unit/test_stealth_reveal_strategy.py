...
from business.stealth_reveal_strategy import (
    ...
    compute_reveal_strategy_slice_size,
    get_reveal_strategy,
)
...
def test_adaptive_basic_proportional():
    ...
    # multiplier=0.1 -> 1.0
    assert _next(s, _order()) == pytest.approx(1.0)
...
def test_adaptive_capped_by_max_pct():
    ...
    assert _next(s, _order()) == pytest.approx(3.0)
...
def test_adaptive_capped_by_remaining():
    ...
    assert _next(s, _order(remaining_size=2.0)) == 2.0


def test_adaptive_zero_when_remaining_zero():
    s = _adaptive()
    assert _next(s, _order(remaining_size=0.0)) == 0.0
...
def test_iceberg_first_slice_uses_first_tranche():
    s = _iceberg()
    assert _next(s, _order()) == pytest.approx(2.5)
...
def test_iceberg_advances_to_next_tranche_after_full_fill():
    ...
    assert _next(s, o) == pytest.approx(2.5)
...
def test_iceberg_partial_fill_then_external_cancel_fills_gap():
    ...
    assert _next(s, o) == pytest.approx(2.5 - 1.8)
...
def test_iceberg_partial_fill_past_first_tranche():
    ...
    assert _next(s, o) == pytest.approx(5.0 - 2.6)

"""Backend-owned stealth manager-invocation policy proof service."""
...
PROOF_ROUTE = "/api/v1/stealth/orders/{stealth_order_id}/manager-invocation-policy-proofs"
READBACK_ROUTE = "/api/v1/stealth/orders/{stealth_order_id}/manager-invocation-policy"
PROOF_METHOD = "POST"
PROOF_SERVICE_METHOD = "record_stealth_manager_invocation_policy_proof"


@dataclass(frozen=True)
class GuardedManagerPolicyCommand:
    """Allowed command context for manager-invocation policy proof evidence."""
...
GUARDED_MANAGER_POLICY_COMMANDS: tuple[GuardedManagerPolicyCommand, ...] = (
    GuardedManagerPolicyCommand(
...
class StealthManagerInvocationPolicyError(ValueError):
    """Raised when stealth manager-invocation policy proof evidence is invalid."""


class AdminApiStealthManagerInvocationPolicyService:
    """Service boundary for append-only manager-invocation policy evidence."""

    def record_proof(
        self,
        *,
        proof_store: FileStealthManagerInvocationPolicyProofStore,
        ...
        self._validate_required({
            ...
            "manager_policy_ref": body.manager_policy_ref,
            "mutation_lock_policy_ref": body.mutation_lock_policy_ref,
            "exchange_reality_policy_ref": body.exchange_reality_policy_ref,
        ...
        proof_id = body.manager_policy_proof_id or _stable_id(
            "stealth-manager-policy-proof",
            route=PROOF_ROUTE,
            stealth_order_id=stealth_order_id,
        ...
        if proof_store.find_by_proof_id(proof_id) is not None:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy proof already exists."
            )

        record = StealthManagerInvocationPolicyProofRecord(
            manager_policy_proof_id=proof_id,
    ...
    def _validate_required(fields: dict[str, str | None]) -> None:
        ...
        if missing:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy proof is missing required fields: "
                + ", ".join(missing)
            )
    ...
    def _validate_guarded_command_context(
        *,
        ...
        guarded_command = next(
            (
                command
                for command in GUARDED_MANAGER_POLICY_COMMANDS
        ...
        if failed:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation guarded command context did not match: "
                + ", ".join(failed)
            )
    ...
    def _validate_safe_proof(
        ...
        if not body.dry_run:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy proof must be dry-run evidence."
            )
        if body.manual_live_acknowledgement:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy proof cannot include live acknowledgement."
            )
    ...
    def _validate_route_inventory(
        *,
        ...
        if inventory_item is None:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy route is missing from route inventory."
            )
        if inventory_item.action_class != AdminApiActionClass.LOCAL_STATE_MUTATION:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy route must be a local-state mutation."
            )
        if inventory_item.permission != AdminApiPermission.STEALTH_MANAGER_POLICY_RECORD:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy route must require "
                "stealth_manager_policy:record."
            )
        if inventory_item.shared_method != service_method:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy service method does not match route inventory."
            )
    ...
    def _validate_admission_prerequisites(
        *,
        ...
        if failed:
            raise StealthManagerInvocationPolicyError(
                "Stealth manager-invocation policy prerequisites did not pass: "
                + ", ".join(failed)
            )

"""Durable stealth post-write reconciliation execution-policy proof helpers."""
...
class StealthPostWriteReconciliationExecutionPolicyProofRecord(BaseModel):
    """Append-only backend post-write reconciliation execution policy evidence."""

    model_config = ConfigDict(extra="forbid")

    post_write_reconciliation_policy_proof_id: str = Field(min_length=1)
...
class FileStealthPostWriteReconciliationExecutionPolicyProofStore:
    """Append-only JSONL stealth post-write reconciliation policy store."""
    ...
    def __init__(self, path: Path | str | None = None) -> None:
        configured_path = (
            ...
            / "admin_api_stealth_post_write_reconciliation_execution_policy_proofs.jsonl"
        )
    ...
    def append(
        ...
        with self._lock:
            ...
            return record.post_write_reconciliation_policy_proof_id

    def read_recent(
        self,
        *,
        limit: int = 100,
    ) -> list[StealthPostWriteReconciliationExecutionPolicyProofRecord]:
        normalized_limit = max(1, min(limit, 500))
        with self._lock:
            if not self.path.exists():
                return []
            lines = self.path.read_text(encoding="utf-8").splitlines()
        records: list[StealthPostWriteReconciliationExecutionPolicyProofRecord] = []
        for line in reversed(lines):
            if not line.strip():
                continue
            try:
                records.append(
                    StealthPostWriteReconciliationExecutionPolicyProofRecord.model_validate_json(
                        line
                    )
                )
            except ValueError:
    ...
    def find_by_proof_id(
        ...
        """Return the latest execution-policy proof record for the id."""
    ...
    def read_for_stealth_order_id(
        ...
        *,
        ...
        """Return recent policy records for one stealth order."""

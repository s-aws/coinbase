"""Backend-owned stealth reveal-trigger proof service."""
...
READBACK_ROUTE = "/api/v1/stealth/orders/{stealth_order_id}/reveal-trigger-proof"
PROOF_METHOD = "POST"
PROOF_SERVICE_METHOD = "record_stealth_reveal_trigger_proof"
...
class StealthRevealTriggerProofError(ValueError):
    """Raised when stealth reveal-trigger proof evidence is invalid."""


class AdminApiStealthRevealTriggerProofService:
    """Service boundary for append-only stealth reveal-trigger proof evidence."""

    def record_proof(
        self,
        *,
        proof_store: FileStealthRevealTriggerProofStore,
        ...
        self._validate_required({
            ...
            "reveal_condition_ref": body.reveal_condition_ref,
            "trigger_evidence_ref": body.trigger_evidence_ref,
            "condition_snapshot_ref": body.condition_snapshot_ref,
        ...
        proof_id = body.reveal_trigger_proof_id or _stable_id(
            "stealth-reveal-trigger-proof",
            route=PROOF_ROUTE,
            stealth_order_id=stealth_order_id,
        ...
        if proof_store.find_by_proof_id(proof_id) is not None:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof already exists."
            )

        record = StealthRevealTriggerProofRecord(
            reveal_trigger_proof_id=proof_id,
    ...
    def _validate_required(fields: dict[str, str | None]) -> None:
        ...
        if missing:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof is missing required fields: "
                + ", ".join(missing)
            )
    ...
    def _validate_guarded_command_context(
        *,
        ...
        if failed:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger guarded command context did not match: "
                + ", ".join(failed)
            )

    @staticmethod
    def _validate_safe_proof(body: StealthRevealTriggerProofRequest) -> None:
        if not body.dry_run:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof must be recorded as dry-run evidence."
            )
        if body.manual_live_acknowledgement:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof cannot include live acknowledgement."
            )
    ...
    def _validate_route_inventory(
        *,
        ...
        if inventory_item is None:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof route is missing from route inventory."
            )
        if inventory_item.action_class != AdminApiActionClass.LOCAL_STATE_MUTATION:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof route must be a local-state mutation."
            )
        if inventory_item.permission != AdminApiPermission.STEALTH_REVEAL_TRIGGER_RECORD:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof route must require "
                "stealth_reveal_trigger:record."
            )
        if inventory_item.shared_method != service_method:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof service method does not match route inventory."
            )
    ...
    def _validate_admission_prerequisites(
        *,
        ...
        if failed:
            raise StealthRevealTriggerProofError(
                "Stealth reveal-trigger proof prerequisites did not pass: "
                + ", ".join(failed)
            )

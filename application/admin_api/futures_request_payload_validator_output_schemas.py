"""Disabled futures/perpetual request payload validator output-schema registry.

These rows are backend-owned evidence for future validator output schemas. They
...
class FuturesRequestPayloadValidatorOutputSchemaContract:
    """One disabled output-schema row for a futures payload validator."""
    ...
    @property
    def output_schema_field_refs(self) -> tuple[str, ...]:
        return (
            f"{self.validator_output_schema_ref}.request_payload_contract_ref",
            f"{self.validator_output_schema_ref}.validation_gate_ref",
            f"{self.validator_output_schema_ref}.validation_evidence_ref",
            f"{self.validator_output_schema_ref}.validator_contract_ref",
            f"{self.validator_output_schema_ref}.runtime_evidence_snapshot",
        )
    ...
    def detail(self) -> str:
        return (
            ...
            f"backend-owned validator output schema {self.validator_output_schema_ref} "
            "before any payload validator can be registered or run. The schema "
            "row is blocked display evidence only."
        )


FUTURES_REQUEST_PAYLOAD_VALIDATOR_OUTPUT_SCHEMA_CONTRACTS: tuple[
    FuturesRequestPayloadValidatorOutputSchemaContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidatorOutputSchemaContract(validator_contract=contract)
    for contract in FUTURES_REQUEST_PAYLOAD_VALIDATOR_CONTRACTS
)


def iter_futures_request_payload_validator_output_schemas(
    command: AdminFuturesCommandAction,
) -> Iterator[FuturesRequestPayloadValidatorOutputSchemaContract]:
    """Yield disabled request-payload validator output schemas for one command."""

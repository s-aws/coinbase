"""Disabled futures/perpetual validation-record position semantics registry.

These rows consume the existing semantic artifact runtime-evidence acceptance
chain and expose the first futures-specific semantic contract gap: position
...
class FuturesRequestPayloadValidationRecordPositionSemanticContract:
    """One disabled position-semantics row for a futures validation record."""
    ...
    position_semantics_contract_available: bool = False
    position_semantics_contract_ready: bool = False
    position_identity_bound: bool = False
    position_scope_bound: bool = False
    position_side_derivation_bound: bool = False
    position_size_bound: bool = False
    position_notional_bound: bool = False
    runtime_position_evidence_observed: bool = False
    runtime_evidence_satisfies_position_semantics: bool = False
    ...
    @property
    def position_semantics_ref(self) -> str:
        return self.semantic_ref

    @property
    def position_semantics_contract_ref(self) -> str:
        return (
    ...
    def required_backend_contract(self) -> str:
        return self.position_semantics_contract_ref

    @property
    def missing_backend_contract(self) -> str:
        return self.position_semantics_ref

    @property
    def missing_reason(self) -> str:
        return (
            "backend-owned futures/perpetual position semantics contract is "
            ...
            dict.fromkeys(
                (
                    *self.semantic_artifact_runtime_evidence_acceptance_contract.required_evidence_refs,
                    self.position_semantics_ref,
                    self.position_semantics_contract_ref,
                    f"{self.position_semantics_contract_ref}.position_key",
                    f"{self.position_semantics_contract_ref}.product_id",
                    f"{self.position_semantics_contract_ref}.side",
                    f"{self.position_semantics_contract_ref}.size",
                    f"{self.position_semantics_contract_ref}.notional",
                    AdminFuturesCommandEvidenceRoute.FUTURES_POSITIONS.value,
                    AdminFuturesCommandEvidenceRoute.FUTURES_POSITION_DETAIL.value,
                    f"{self.position_semantics_contract_ref}_contextless_review",
                )
            )
        )
    ...
    def detail(self) -> str:
        return (
            ...
            "position semantics for futures/perpetual validation records. The "
            "contract must bind position identity, configured/observed scope, "
            "side derivation, size, notional, and read-route evidence before "
            "position semantics can satisfy execution eligibility. This row is "
            "not a validator, command admission, Coinbase call, reconciliation "
            "execution, state mutation, browser authority, or BFF execution "
            "authority."
        )


FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_POSITION_SEMANTIC_CONTRACTS: tuple[
    FuturesRequestPayloadValidationRecordPositionSemanticContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidationRecordPositionSemanticContract(
    ...
    if contract.semantic_artifact == AdminFuturesCommandSemanticArtifact.POSITION_SEMANTICS
)


def iter_futures_request_payload_validation_record_position_semantics(
    command: AdminFuturesCommandAction,
) -> Iterator[FuturesRequestPayloadValidationRecordPositionSemanticContract]:
    """Yield disabled position-semantics contracts for one futures command."""

"""Disabled futures/perpetual validation-record liquidation semantics registry.

These rows consume the existing semantic artifact runtime-evidence acceptance
chain and expose the next futures-specific semantic contract gap: liquidation
...
class FuturesRequestPayloadValidationRecordLiquidationSemanticContract:
    """One disabled liquidation-semantics row for a futures validation record."""
    ...
    liquidation_semantics_contract_available: bool = False
    liquidation_semantics_contract_ready: bool = False
    liquidation_buffer_bound: bool = False
    liquidation_price_bound: bool = False
    liquidation_distance_bound: bool = False
    liquidation_threshold_bound: bool = False
    runtime_liquidation_evidence_observed: bool = False
    runtime_evidence_satisfies_liquidation_semantics: bool = False
    ...
    @property
    def liquidation_semantics_ref(self) -> str:
        return self.semantic_ref

    @property
    def liquidation_semantics_contract_ref(self) -> str:
        return (
    ...
    def required_backend_contract(self) -> str:
        return self.liquidation_semantics_contract_ref

    @property
    def missing_backend_contract(self) -> str:
        return self.liquidation_semantics_ref

    @property
    def missing_reason(self) -> str:
        return (
            "backend-owned futures/perpetual liquidation semantics contract is "
            ...
            dict.fromkeys(
                (
                    *self.semantic_artifact_runtime_evidence_acceptance_contract.required_evidence_refs,
                    self.liquidation_semantics_ref,
                    self.liquidation_semantics_contract_ref,
                    f"{self.liquidation_semantics_contract_ref}.liquidation_buffer",
                    f"{self.liquidation_semantics_contract_ref}.liquidation_price",
                    f"{self.liquidation_semantics_contract_ref}.liquidation_distance",
                    f"{self.liquidation_semantics_contract_ref}.liquidation_threshold",
                    AdminFuturesCommandEvidenceRoute.FUTURES_ACCOUNT.value,
                    AdminFuturesCommandEvidenceRoute.FUTURES_RISK_PROOFS.value,
                    f"{self.liquidation_semantics_contract_ref}_contextless_review",
                )
            )
        )
    ...
    def detail(self) -> str:
        return (
            ...
            "liquidation semantics for futures/perpetual validation records. "
            "The contract must bind liquidation buffer, liquidation price, "
            "liquidation distance, liquidation threshold, and risk-proof "
            "evidence before liquidation semantics can satisfy execution "
...
FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_LIQUIDATION_SEMANTIC_CONTRACTS: tuple[
    FuturesRequestPayloadValidationRecordLiquidationSemanticContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidationRecordLiquidationSemanticContract(
    ...
    if contract.semantic_artifact == AdminFuturesCommandSemanticArtifact.LIQUIDATION_SEMANTICS
)


def iter_futures_request_payload_validation_record_liquidation_semantics(
    command: AdminFuturesCommandAction,
) -> Iterator[FuturesRequestPayloadValidationRecordLiquidationSemanticContract]:
    """Yield disabled liquidation-semantics contracts for one futures command."""

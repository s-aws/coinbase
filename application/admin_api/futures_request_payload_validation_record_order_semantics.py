"""Disabled futures/perpetual validation-record order semantics registry.

These rows consume the existing semantic artifact runtime-evidence acceptance
chain and expose the next futures-specific semantic contract gap: order
...
class FuturesRequestPayloadValidationRecordOrderSemanticContract:
    """One disabled order-semantics row for a futures validation record."""
    ...
    order_semantics_contract_available: bool = False
    order_semantics_contract_ready: bool = False
    order_identity_bound: bool = False
    order_side_bound: bool = False
    order_size_bound: bool = False
    order_price_bound: bool = False
    order_type_bound: bool = False
    runtime_order_evidence_observed: bool = False
    runtime_evidence_satisfies_order_semantics: bool = False
    ...
    @property
    def order_semantics_ref(self) -> str:
        return self.semantic_ref

    @property
    def order_semantics_contract_ref(self) -> str:
        return (
    ...
    def required_backend_contract(self) -> str:
        return self.order_semantics_contract_ref

    @property
    def missing_backend_contract(self) -> str:
        return self.order_semantics_ref

    @property
    def missing_reason(self) -> str:
        return (
            "backend-owned futures/perpetual order semantics contract is "
            ...
            dict.fromkeys(
                (
                    *self.semantic_artifact_runtime_evidence_acceptance_contract.required_evidence_refs,
                    self.order_semantics_ref,
                    self.order_semantics_contract_ref,
                    f"{self.order_semantics_contract_ref}.order_identity",
                    f"{self.order_semantics_contract_ref}.order_side",
                    f"{self.order_semantics_contract_ref}.order_size",
                    f"{self.order_semantics_contract_ref}.order_price",
                    f"{self.order_semantics_contract_ref}.order_type",
                    AdminFuturesCommandEvidenceRoute.FUTURES_ACCOUNT.value,
                    AdminFuturesCommandEvidenceRoute.FUTURES_RISK_PROOFS.value,
                    f"{self.order_semantics_contract_ref}_contextless_review",
                )
            )
        )
    ...
    def detail(self) -> str:
        return (
            ...
            "order semantics for futures/perpetual validation records. "
            "The contract must bind order identity, side, size, price, type, "
            "futures-account evidence, and risk-proof evidence before order "
            "semantics can satisfy execution eligibility. This row is not a "
...
FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_ORDER_SEMANTIC_CONTRACTS: tuple[
    FuturesRequestPayloadValidationRecordOrderSemanticContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidationRecordOrderSemanticContract(
    ...
    if contract.semantic_artifact == AdminFuturesCommandSemanticArtifact.ORDER_SEMANTICS
)


def iter_futures_request_payload_validation_record_order_semantics(
    command: AdminFuturesCommandAction,
) -> Iterator[FuturesRequestPayloadValidationRecordOrderSemanticContract]:
    """Yield disabled order-semantics contracts for one futures command."""

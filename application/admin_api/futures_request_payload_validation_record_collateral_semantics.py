"""Disabled futures/perpetual validation-record collateral semantics registry.

These rows consume the existing semantic artifact runtime-evidence acceptance
chain and expose the next futures-specific semantic contract gap: collateral
...
class FuturesRequestPayloadValidationRecordCollateralSemanticContract:
    """One disabled collateral-semantics row for a futures validation record."""
    ...
    @property
    def collateral_semantics_ref(self) -> str:
        return self.semantic_ref

    @property
    def collateral_semantics_contract_ref(self) -> str:
        return (
    ...
    def required_backend_contract(self) -> str:
        return self.collateral_semantics_contract_ref

    @property
    def missing_backend_contract(self) -> str:
        return self.collateral_semantics_ref

    @property
    def missing_reason(self) -> str:
        return (
            "backend-owned futures/perpetual collateral semantics contract is "
            ...
            dict.fromkeys(
                (
                    *self.semantic_artifact_runtime_evidence_acceptance_contract.required_evidence_refs,
                    self.collateral_semantics_ref,
                    self.collateral_semantics_contract_ref,
                    f"{self.collateral_semantics_contract_ref}.available_collateral",
                    f"{self.collateral_semantics_contract_ref}.collateral_currency",
                    f"{self.collateral_semantics_contract_ref}.collateral_requirement",
                    f"{self.collateral_semantics_contract_ref}.collateral_source",
                    AdminFuturesCommandEvidenceRoute.FUTURES_ACCOUNT.value,
                    AdminFuturesCommandEvidenceRoute.FUTURES_RISK_PROOFS.value,
                    f"{self.collateral_semantics_contract_ref}_contextless_review",
                )
            )
        )
    ...
    def detail(self) -> str:
        return (
            ...
            "collateral semantics for futures/perpetual validation records. The "
            "contract must bind available collateral, collateral currency, "
            "collateral requirement, collateral source, and risk-proof evidence "
            "before collateral semantics can satisfy execution eligibility. This "
...
FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_COLLATERAL_SEMANTIC_CONTRACTS: tuple[
    FuturesRequestPayloadValidationRecordCollateralSemanticContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidationRecordCollateralSemanticContract(
    ...
    if contract.semantic_artifact == AdminFuturesCommandSemanticArtifact.COLLATERAL_SEMANTICS
)


def iter_futures_request_payload_validation_record_collateral_semantics(
    command: AdminFuturesCommandAction,
) -> Iterator[FuturesRequestPayloadValidationRecordCollateralSemanticContract]:
    """Yield disabled collateral-semantics contracts for one futures command."""

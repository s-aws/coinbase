"""Disabled futures/perpetual semantic definition review-output registry.

These rows name the backend-owned output evidence required before a
contextless semantic definition review can be accepted. They are evidence only
and do not validate payloads, admit commands, call Coinbase, execute
...
from .futures_request_payload_validation_record_semantic_artifact_definition_review_inputs import (
...
class FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputContract:
    """One missing output-evidence row for a futures semantic definition review."""

    semantic_artifact_definition_review_input_contract: (
        FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewInputContract
    )
    ...
    def command(self) -> AdminFuturesCommandAction:
        return self.semantic_artifact_definition_review_input_contract.command

    @property
    def field(self) -> AdminFuturesCommandRequestField:
        return self.semantic_artifact_definition_review_input_contract.field

    @property
    def blocker(self) -> AdminFuturesCommandExecutionEligibilityBlocker:
        return self.semantic_artifact_definition_review_input_contract.blocker

    @property
    def semantic_artifact(self) -> AdminFuturesCommandSemanticArtifact:
        return self.semantic_artifact_definition_review_input_contract.semantic_artifact

    @property
    def validation_record_execution_eligibility_contract_ref(self) -> str:
        return (
            self.semantic_artifact_definition_review_input_contract
    ...
    def semantic_ref(self) -> str:
        return self.semantic_artifact_definition_review_input_contract.semantic_ref

    @property
    def semantic_artifact_ref(self) -> str:
        return self.semantic_artifact_definition_review_input_contract.semantic_artifact_ref
    ...
    def semantic_artifact_definition_review_output_contract_ref(self) -> str:
        return (
            ...
            f"{self.command.value}_{self.field.value}_{self.semantic_artifact.value}_definition_review_output"
        )
    ...
    def required_backend_contract(self) -> str:
        return self.semantic_artifact_definition_review_output_contract_ref

    @property
    def missing_backend_contract(self) -> str:
        return self.semantic_artifact_definition_review_output_ref

    @property
    def missing_reason(self) -> str:
        return self.semantic_artifact_definition_review_input_contract.missing_reason
    ...
    def required_evidence_refs(self) -> tuple[str, ...]:
        return tuple(
            dict.fromkeys(
                (
                    *self.semantic_artifact_definition_review_input_contract.required_evidence_refs,
                    self.semantic_artifact_definition_review_output_ref,
                    self.semantic_artifact_definition_review_output_contract_ref,
                )
            )
        )
    ...
    def detail(self) -> str:
        return (
            ...
            f"output evidence for contextless {self.semantic_artifact.value} "
            "definition review; no validation, admission, Coinbase, "
            "reconciliation, state, browser, or BFF authority."
        )


FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_SEMANTIC_ARTIFACT_DEFINITION_REVIEW_OUTPUT_CONTRACTS: tuple[
    FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputContract(
        semantic_artifact_definition_review_input_contract=contract,
    )
    for contract in (
        FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_SEMANTIC_ARTIFACT_DEFINITION_REVIEW_INPUT_CONTRACTS
    )
...
def iter_futures_request_payload_validation_record_semantic_artifact_definition_review_outputs(
    command: AdminFuturesCommandAction,
) -> Iterator[
    FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputContract
]:
    """Yield disabled semantic definition review-output rows for one command."""

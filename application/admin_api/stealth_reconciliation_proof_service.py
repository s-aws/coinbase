"""Backend-owned stealth reconciliation proof service."""
...
PROOF_ROUTE = "/api/v1/stealth/orders/{stealth_order_id}/reconciliation-proofs"
READBACK_ROUTE = "/api/v1/stealth/orders/{stealth_order_id}/reconciliation-proof"
PROOF_METHOD = "POST"
PROOF_SERVICE_METHOD = "record_stealth_reconciliation_proof"
...
class StealthReconciliationProofError(ValueError):
    """Raised when stealth reconciliation proof evidence is invalid."""


class AdminApiStealthReconciliationProofService:
    """Service boundary for append-only stealth reconciliation proof evidence."""

    def record_proof(
        self,
        *,
        proof_store: FileStealthReconciliationProofStore,
        ...
        self._validate_required({
            ...
            "reconciliation_evidence_ref": body.reconciliation_evidence_ref,
        ...
        proof_id = body.reconciliation_proof_id or _stable_id(
            "stealth-reconciliation-proof",
            route=PROOF_ROUTE,
            stealth_order_id=stealth_order_id,
        ...
        if proof_store.find_by_proof_id(proof_id) is not None:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof already exists."
            )

        record = StealthReconciliationProofRecord(
            reconciliation_proof_id=proof_id,
    ...
    def _validate_required(fields: dict[str, str | None]) -> None:
        ...
        if missing:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof is missing required fields: "
                + ", ".join(missing)
            )
    ...
    def _validate_guarded_command_context(
        *,
        ...
        if failed:
            raise StealthReconciliationProofError(
                "Stealth reconciliation guarded command context did not match: "
                + ", ".join(failed)
            )

    @staticmethod
    def _validate_safe_proof(body: StealthReconciliationProofRequest) -> None:
        if not body.dry_run:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof must be recorded as dry-run evidence."
            )
        if body.manual_live_acknowledgement:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof cannot include live acknowledgement."
            )
    ...
    def _validate_route_inventory(
        *,
        ...
        if inventory_item is None:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof route is missing from route inventory."
            )
        if inventory_item.action_class != AdminApiActionClass.LOCAL_STATE_MUTATION:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof route must be a local-state mutation."
            )
        if inventory_item.permission != AdminApiPermission.STEALTH_RECONCILIATION_RECORD:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof route must require "
                "stealth_reconciliation:record."
            )
        if inventory_item.shared_method != service_method:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof service method does not match route inventory."
            )
        ...
        *,
        ...
        checks = {
            ...
            "required_permission": (
                ...
                == AdminApiPermission.STEALTH_RECONCILIATION_RECORD
            ),
        ...
        if failed:
            raise StealthReconciliationProofError(
                "Stealth reconciliation proof prerequisites did not pass: "
                + ", ".join(failed)
            )

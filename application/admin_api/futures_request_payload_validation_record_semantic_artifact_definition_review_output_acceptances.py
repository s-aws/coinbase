"""Disabled futures/perpetual semantic definition review-output acceptance registry.

These rows name the backend-owned acceptance/readback evidence required before
a contextless semantic definition review output can be accepted. They are
...
from .futures_request_payload_validation_record_semantic_artifact_definition_review_outputs import (
    FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_SEMANTIC_ARTIFACT_DEFINITION_REVIEW_OUTPUT_CONTRACTS,
    FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputContract,
)


@dataclass(frozen=True)
class FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputAcceptanceContract:
    """One missing acceptance/readback row for a futures semantic review output."""

    semantic_artifact_definition_review_output_contract: (
        FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputContract
    )
    ...
    def command(self) -> AdminFuturesCommandAction:
        return self.semantic_artifact_definition_review_output_contract.command

    @property
    def field(self) -> AdminFuturesCommandRequestField:
        return self.semantic_artifact_definition_review_output_contract.field

    @property
    def blocker(self) -> AdminFuturesCommandExecutionEligibilityBlocker:
        return self.semantic_artifact_definition_review_output_contract.blocker

    @property
    def semantic_artifact(self) -> AdminFuturesCommandSemanticArtifact:
        return self.semantic_artifact_definition_review_output_contract.semantic_artifact

    @property
    def validation_record_execution_eligibility_contract_ref(self) -> str:
        return (
            self.semantic_artifact_definition_review_output_contract
    ...
    def semantic_ref(self) -> str:
        return self.semantic_artifact_definition_review_output_contract.semantic_ref

    @property
    def semantic_artifact_ref(self) -> str:
        return self.semantic_artifact_definition_review_output_contract.semantic_artifact_ref
    ...
    def semantic_artifact_definition_review_output_acceptance_ref(self) -> str:
        return (
            f"{self.semantic_artifact_definition_review_output_ref}_acceptance"
        )
    ...
    def semantic_artifact_definition_review_output_acceptance_contract_ref(self) -> str:
        return (
            ...
            f"{self.command.value}_{self.field.value}_{self.semantic_artifact.value}_definition_review_output_acceptance"
        )
    ...
    def required_backend_contract(self) -> str:
        return self.semantic_artifact_definition_review_output_acceptance_contract_ref

    @property
    def missing_backend_contract(self) -> str:
        return self.semantic_artifact_definition_review_output_acceptance_ref

    @property
    def missing_reason(self) -> str:
        return self.semantic_artifact_definition_review_output_contract.missing_reason
    ...
    def required_evidence_refs(self) -> tuple[str, ...]:
        return tuple(
            dict.fromkeys(
                (
                    *self.semantic_artifact_definition_review_output_contract.required_evidence_refs,
                    self.semantic_artifact_definition_review_output_acceptance_ref,
                    self.semantic_artifact_definition_review_output_acceptance_contract_ref,
                )
            )
        )
    ...
    def detail(self) -> str:
        return (
            ...
            f"acceptance/readback evidence for contextless {self.semantic_artifact.value} "
            "definition review output; no validation, admission, Coinbase, "
            "reconciliation, state, browser, or BFF authority."
        )


FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_SEMANTIC_ARTIFACT_DEFINITION_REVIEW_OUTPUT_ACCEPTANCE_CONTRACTS: tuple[
    FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputAcceptanceContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputAcceptanceContract(
        semantic_artifact_definition_review_output_contract=contract,
    )
    for contract in (
        FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_SEMANTIC_ARTIFACT_DEFINITION_REVIEW_OUTPUT_CONTRACTS
    )
...
def iter_futures_request_payload_validation_record_semantic_artifact_definition_review_output_acceptances(
    command: AdminFuturesCommandAction,
) -> Iterator[
    FuturesRequestPayloadValidationRecordSemanticArtifactDefinitionReviewOutputAcceptanceContract
]:
    """Yield disabled semantic definition review-output acceptance rows."""

"""Disabled futures/perpetual validation-record funding semantics registry.

These rows consume the existing semantic artifact runtime-evidence acceptance
chain and expose the next futures-specific semantic contract gap: funding
...
class FuturesRequestPayloadValidationRecordFundingSemanticContract:
    """One disabled funding-semantics row for a futures validation record."""
    ...
    @property
    def funding_semantics_ref(self) -> str:
        return self.semantic_ref

    @property
    def funding_semantics_contract_ref(self) -> str:
        return (
    ...
    def required_backend_contract(self) -> str:
        return self.funding_semantics_contract_ref

    @property
    def missing_backend_contract(self) -> str:
        return self.funding_semantics_ref

    @property
    def missing_reason(self) -> str:
        return (
            "backend-owned futures/perpetual funding semantics contract is "
            ...
            dict.fromkeys(
                (
                    *self.semantic_artifact_runtime_evidence_acceptance_contract.required_evidence_refs,
                    self.funding_semantics_ref,
                    self.funding_semantics_contract_ref,
                    f"{self.funding_semantics_contract_ref}.funding_rate",
                    f"{self.funding_semantics_contract_ref}.funding_fee",
                    f"{self.funding_semantics_contract_ref}.funding_interval",
                    f"{self.funding_semantics_contract_ref}.funding_cost",
                    AdminFuturesCommandEvidenceRoute.FUTURES_ACCOUNT.value,
                    AdminFuturesCommandEvidenceRoute.FUTURES_RISK_PROOFS.value,
                    f"{self.funding_semantics_contract_ref}_contextless_review",
                )
            )
        )
    ...
    def detail(self) -> str:
        return (
            ...
            "funding semantics for futures/perpetual validation records. "
            "The contract must bind funding rate, funding fee, funding "
            "interval, funding cost, futures-account evidence, and risk-proof "
            "evidence before funding semantics can satisfy execution "
            "eligibility. This row is not a validator, command admission, Coinbase call, "
            "reconciliation execution, state mutation, browser authority, or "
            "BFF execution authority."
        )


FUTURES_REQUEST_PAYLOAD_VALIDATION_RECORD_FUNDING_SEMANTIC_CONTRACTS: tuple[
    FuturesRequestPayloadValidationRecordFundingSemanticContract,
    ...,
] = tuple(
    FuturesRequestPayloadValidationRecordFundingSemanticContract(
    ...
    if contract.semantic_artifact == AdminFuturesCommandSemanticArtifact.FUNDING_SEMANTICS
)


def iter_futures_request_payload_validation_record_funding_semantics(
    command: AdminFuturesCommandAction,
) -> Iterator[FuturesRequestPayloadValidationRecordFundingSemanticContract]:
    """Yield disabled funding-semantics contracts for one futures command."""

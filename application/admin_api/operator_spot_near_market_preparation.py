"""No-retry Coinbase evidence reader for one near-market candidate proposal."""
...
from .operator_spot_near_market_policy import (
...
class NearMarketPreparationOutcome(str, Enum):
...
class NearMarketPreparationResult:
    outcome: NearMarketPreparationOutcome
    diagnostic_code: str
    completed_categories: tuple[str, ...]
    coinbase_api_call_count: int | None
    call_count_exact: bool
    evidence_sha256: str | None
    plan: NearMarketBuyPlan | None
...
def _result(
    *,
    ...
    completed: list[str],
    call_count: int | None,
    plan: NearMarketBuyPlan | None = None,
    ...
    return NearMarketPreparationResult(
        outcome=outcome,
        diagnostic_code=diagnostic_code,
...
def _blocked(
    ...
    *,
    ...
    return _result(
        outcome=NearMarketPreparationOutcome.BLOCKED,
...
def _unknown(completed: list[str]) -> NearMarketPreparationResult:
    return _result(
        outcome=NearMarketPreparationOutcome.UNKNOWN,
...
def run_near_market_candidate_preparation(
    *,
    rest_client: Any,
    ...
    if not (
        str(permissions.get("portfolio_uuid") or permissions.get("portfolio_id") or "").strip()
    ...
    trade = _mapping(trades[0]) if isinstance(trades, list) and len(trades) == 1 else {}
    ...
    try:
        ...
        plan = derive_near_market_buy_plan(
            product_id="BTC-USDC",

"""Canonical Goal 10 exchange adapter for one Futures manual lifecycle.

The adapter owns no durable state. Callers must durably claim each allowance
through the Goal 10 repository before passing the corresponding ``before_call``
callback. Raw Preview and exchange identifiers remain ephemeral; public
evidence contains only SHA-256 bindings and fixed value-blind diagnostics.
"""
...
FUTURES_MANUAL_PRODUCT_ID = "AVP-20DEC30-CDE"
FUTURES_MANUAL_CONTRACT_COUNT = Decimal("1")
FUTURES_MANUAL_OPENING_CAP_USDC = Decimal("100")
FUTURES_MANUAL_EXPOSURE_CAP_USDC = Decimal("150")
FUTURES_MANUAL_TURNOVER_CAP_USDC = Decimal("300")
...
class FuturesManualPreviewExecution:
    outcome: AdminFuturesManualCallOutcome
    diagnostic_code: str
    preview_id_sha256: str | None
    public_evidence: dict[str, Any]
    private_preview_id: str | None = None


@dataclass(frozen=True, slots=True)
class FuturesManualCreateExecution:
    outcome: AdminFuturesManualCallOutcome
    diagnostic_code: str
...
@dataclass(frozen=True, slots=True)
class FuturesManualReconciliationExecution:
    outcome: AdminFuturesManualCallOutcome
    diagnostic_code: str
...
@dataclass(frozen=True, slots=True)
class FuturesManualCancelExecution:
    outcome: AdminFuturesManualCallOutcome
    diagnostic_code: str
    exchange_order_id_sha256: str
    public_evidence: dict[str, Any]
...
def _shallow_mapping(value: Any) -> dict[str, Any] | None:
    """Copy one SDK object without recursively converting unknown values."""
...
def _validated_candidate(candidate: Mapping[str, Any]) -> dict[str, str]:
    ...
    if (
        normalized.get("product_id") != FUTURES_MANUAL_PRODUCT_ID
        ...
        or turnover is None
        or opening >= FUTURES_MANUAL_OPENING_CAP_USDC
        or exposure >= FUTURES_MANUAL_EXPOSURE_CAP_USDC
        or buffered_close >= FUTURES_MANUAL_EXPOSURE_CAP_USDC
        or turnover >= FUTURES_MANUAL_TURNOVER_CAP_USDC
        or normalized.get("opening_cap_usdc") != "100"
        or normalized.get("exposure_cap_usdc") != "150"
        or normalized.get("turnover_cap_usdc") != "300"
    ):
        raise ValueError("operator_futures_manual_candidate_invalid")
    return normalized


def _order_configuration(candidate: Mapping[str, str]) -> dict[str, Any]:
    return {
        "limit_limit_gtc": {
            "base_size": candidate["contract_count"],
...
def _unknown_preview() -> FuturesManualPreviewExecution:
    return FuturesManualPreviewExecution(
        ...
        diagnostic_code="operator_futures_manual_preview_outcome_unknown",
...
def _unknown_create() -> FuturesManualCreateExecution:
    return FuturesManualCreateExecution(
        outcome=AdminFuturesManualCallOutcome.UNKNOWN,
        diagnostic_code="operator_futures_manual_create_outcome_unknown",
...
class AdminApiFuturesManualExchangeExecutor:
    """Invoke only the distinct Goal 10 Futures Preview/Create/read/cancel path."""
    ...
    def preview(
        ...
        *,
        ...
        try:
            exact_candidate = _validated_candidate(candidate)
            with canonical_coinbase_execution_scope(
                COINBASE_EXECUTION_SCOPE_FUTURES_PREVIEW
            ):
                raw = self.rest_client.preview_futures_order(
                    product_id=FUTURES_MANUAL_PRODUCT_ID,
                    side="BUY",
        ...
        *,
        ...
        try:
            ...
            if not exact_client_order_id or not exact_preview_id:
                raise ValueError("operator_futures_manual_create_binding_invalid")
            with canonical_coinbase_execution_scope(
                COINBASE_EXECUTION_SCOPE_FUTURES_PLACE
            ):
            ...
            if response.get("success") is False:
                return FuturesManualCreateExecution(
                    outcome=AdminFuturesManualCallOutcome.REJECTED,
                    diagnostic_code=(
                        "operator_futures_manual_create_exchange_rejected"
                    ),
                    exchange_order_id_sha256=None,
                    public_evidence={
                        "raw_response_included": False,
            ...
            if response.get("success") is not True or success is None:
                return _unknown_create()
            exchange_order_id = str(success.get("order_id") or "").strip()
            if (
                not exchange_order_id
                or success.get("product_id") != FUTURES_MANUAL_PRODUCT_ID
                or success.get("side") != "BUY"
                or success.get("client_order_id") != exact_client_order_id
            ):
                return _unknown_create()
        ...
        *,
        ...
        try:
            ...
            if not exact_client_order_id or not exchange_order_id:
                raise ValueError(
                    "operator_futures_manual_reconciliation_binding_invalid"
                )
            ...
            if (
                ...
                or order.get("client_order_id") != exact_client_order_id
                or order.get("product_id") != FUTURES_MANUAL_PRODUCT_ID
                or order.get("side") != "BUY"
            ):
                raise ValueError(
                    "operator_futures_manual_reconciliation_identity_invalid"
                )
            ...
            if order_status not in (
                ...
                raise ValueError(
                    "operator_futures_manual_reconciliation_status_invalid"
                )
    ...
    def cancel(
        ...
        *,
        ...
        try:
            ...
            if not exact_client_order_id or not exchange_order_id:
                raise ValueError(
                    "operator_futures_manual_cancel_binding_invalid"
                )
            with canonical_coinbase_execution_scope(
                COINBASE_EXECUTION_SCOPE_FUTURES_CANCEL
            ):
            ...
            if type(results) is not list or len(results) != 1:
                raise ValueError(
                    "operator_futures_manual_cancel_response_invalid"
                )
            result = _shallow_mapping(results[0])
            if result is None or result.get("order_id") != exchange_order_id:
                raise ValueError(
                    "operator_futures_manual_cancel_identity_invalid"
                )
            if result.get("success") is False:
                return FuturesManualCancelExecution(
                    outcome=AdminFuturesManualCallOutcome.REJECTED,
                    diagnostic_code=(
                        "operator_futures_manual_cancel_exchange_rejected"
                    ),
                    exchange_order_id_sha256=exchange_hash,
                    public_evidence={
                        "client_order_id": exact_client_order_id,
...
__all__ = [
    ...
    "FUTURES_MANUAL_CONTRACT_COUNT",
    "FUTURES_MANUAL_EXPOSURE_CAP_USDC",
    "FUTURES_MANUAL_OPENING_CAP_USDC",
    "FUTURES_MANUAL_PRODUCT_ID",
    "FUTURES_MANUAL_TURNOVER_CAP_USDC",
    "FuturesManualCancelExecution",
    "FuturesManualCreateExecution",
    "FuturesManualPreviewExecution",
    "FuturesManualReconciliationExecution",

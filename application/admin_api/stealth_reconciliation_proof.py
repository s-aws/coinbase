"""Durable stealth reconciliation proof evidence helpers."""
...
class StealthReconciliationProofRecord(BaseModel):
    """Append-only backend stealth reconciliation proof evidence."""
    ...
    mutation_family: AdminApiMutationFamilyType = (
        AdminApiMutationFamilyType.STEALTH_RECONCILIATION_PROOF
    )
...
class FileStealthReconciliationProofStore:
    """Append-only JSONL stealth reconciliation proof store."""

    def __init__(self, path: Path | str | None = None) -> None:
        configured_path = (
            path
            or os.environ.get("COINBASE_ADMIN_API_STEALTH_RECONCILIATION_PROOF_LOG_PATH")
            or Path("runtime_state") / "admin_api_stealth_reconciliation_proofs.jsonl"
        )
        self.path = Path(configured_path)
        self._lock = RLock()

    def append(self, record: StealthReconciliationProofRecord) -> str:
        with self._lock:
            self.path.parent.mkdir(parents=True, exist_ok=True)
            with self.path.open("a", encoding="utf-8") as handle:
                handle.write(record.model_dump_json() + "\n")
            return record.reconciliation_proof_id

    def read_recent(
        self,
        *,
        limit: int = 100,
    ) -> list[StealthReconciliationProofRecord]:
        normalized_limit = max(1, min(limit, 500))
        with self._lock:
            if not self.path.exists():
                return []
            lines = self.path.read_text(encoding="utf-8").splitlines()
        records: list[StealthReconciliationProofRecord] = []
        for line in reversed(lines):
            if not line.strip():
                continue
            try:
                records.append(StealthReconciliationProofRecord.model_validate_json(line))
            except ValueError:
                continue
    ...
    def find_by_proof_id(
        ...
        """Return the latest reconciliation proof record for the id."""
    ...
    def read_for_stealth_order_id(
        ...
        *,
        ...
        """Return recent reconciliation proof records for one stealth order id."""

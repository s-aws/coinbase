"""Durable stealth mutation-claim snapshot proof evidence helpers."""
...
class StealthMutationClaimSnapshotProofRecord(BaseModel):
    """Append-only backend stealth mutation-claim snapshot proof evidence."""

    model_config = ConfigDict(extra="forbid")

    mutation_claim_proof_id: str = Field(min_length=1)
...
class FileStealthMutationClaimProofStore:
    """Append-only JSONL stealth mutation-claim proof store."""

    def __init__(self, path: Path | str | None = None) -> None:
        configured_path = (
            path
            or os.environ.get(
                "COINBASE_ADMIN_API_STEALTH_MUTATION_CLAIM_PROOF_LOG_PATH"
            )
            or Path("runtime_state")
            / "admin_api_stealth_mutation_claim_snapshot_proofs.jsonl"
        )
        self.path = Path(configured_path)
        self._lock = RLock()

    def append(self, record: StealthMutationClaimSnapshotProofRecord) -> str:
        with self._lock:
            self.path.parent.mkdir(parents=True, exist_ok=True)
            with self.path.open("a", encoding="utf-8") as handle:
                handle.write(record.model_dump_json() + "\n")
            return record.mutation_claim_proof_id

    def read_recent(
        self,
        *,
        limit: int = 100,
    ) -> list[StealthMutationClaimSnapshotProofRecord]:
        normalized_limit = max(1, min(limit, 500))
        with self._lock:
            if not self.path.exists():
                return []
            lines = self.path.read_text(encoding="utf-8").splitlines()
        records: list[StealthMutationClaimSnapshotProofRecord] = []
        for line in reversed(lines):
            if not line.strip():
                continue
            try:
                records.append(
                    StealthMutationClaimSnapshotProofRecord.model_validate_json(
                        line
                    )
                )
            except ValueError:
    ...
    def find_by_proof_id(
        ...
        """Return the latest mutation-claim proof record for the id."""

        for record in self.read_recent(limit=500):
            if record.mutation_claim_proof_id == mutation_claim_proof_id:
                return record
        return None
    ...
    def read_for_stealth_order_id(
        ...
        *,
        ...
        """Return recent mutation-claim proof records for one stealth order id."""

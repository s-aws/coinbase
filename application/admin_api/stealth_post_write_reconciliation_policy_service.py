"""Backend-owned stealth post-write reconciliation execution-policy service."""
...
from .stealth_post_write_reconciliation_service import GUARDED_COMMANDS
...
PROOF_METHOD = "POST"
PROOF_SERVICE_METHOD = (
    "record_stealth_post_write_reconciliation_execution_policy_proof"
)


class StealthPostWriteReconciliationExecutionPolicyError(ValueError):
    """Raised when post-write reconciliation execution-policy evidence is invalid."""


class AdminApiStealthPostWriteReconciliationExecutionPolicyService:
    """Service boundary for append-only post-write reconciliation policy evidence."""

    def record_proof(
        self,
        *,
        proof_store: FileStealthPostWriteReconciliationExecutionPolicyProofStore,
        ...
        self._validate_required({
            ...
            "post_write_reconciliation_execution_policy_ref": (
                body.post_write_reconciliation_execution_policy_ref
            ),
            "route_bound_reconciliation_plan_ref": (
                body.route_bound_reconciliation_plan_ref
            ),
            "post_write_execution_journal_policy_ref": (
                body.post_write_execution_journal_policy_ref
            ),
            "post_write_reconciliation_verification_policy_ref": (
                body.post_write_reconciliation_verification_policy_ref
            ),
            "safe_reconciliation_chain_ref": body.safe_reconciliation_chain_ref,
        ...
        proof_id = body.post_write_reconciliation_policy_proof_id or _stable_id(
            "stealth-post-write-reconciliation-policy-proof",
            route=PROOF_ROUTE,
            stealth_order_id=stealth_order_id,
        ...
        if proof_store.find_by_proof_id(proof_id) is not None:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy proof already exists."
            )
    ...
    def _validate_guarded_command_context(
        *,
        ...
        if guarded is None:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy guarded "
                "mutation family is not supported."
            )
        ...
        if failed:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy guarded "
                "command context did not match: " + ", ".join(failed)
            )
    ...
    def _validate_safe_proof(
        ...
        if not body.dry_run:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy proof must "
                "be dry-run evidence."
            )
        if body.manual_live_acknowledgement:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy proof "
                "cannot include live acknowledgement."
            )
    ...
    def _validate_route_inventory(
        *,
        ...
        if inventory_item is None:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy route is "
                "missing from route inventory."
            )
        if inventory_item.action_class != AdminApiActionClass.LOCAL_STATE_MUTATION:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy route must "
                "be a local-state mutation."
            )
        if (
            ...
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy route must "
                "require stealth_post_write_reconciliation_policy:record."
            )
        if inventory_item.shared_method != service_method:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                "Stealth post-write reconciliation execution policy service "
                "method does not match route inventory."
            )
        ...
        *,
        ...
        if failed:
            raise StealthPostWriteReconciliationExecutionPolicyError(
                ...
                "prerequisites did not pass: " + ", ".join(failed)
            )

...
> Verification status: this is a mixed-branch reference. Sections naming files
> or routes absent from the current checkout are historical, not active API
> contracts. Verify every path and handler against code before relying on it.

This file covers API surfaces documented across project branches:
...
- Historical Enterprise Admin API contract (`api/v1/app.py`, when present)
...
- legacy dashboard `place_order` and `cancel_order` messages are handled by
  the compatibility handlers in `dashboard_server.py` in this checkout

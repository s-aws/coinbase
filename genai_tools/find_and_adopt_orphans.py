#!/usr/bin/env python3
"""
Find and Adopt Orphaned Orders Tool

Searches for child orders that don't have a matching parent, then finds
compatible parent orders based on:
- Same product_id
- Same side (BUY/SELL)
...
Usage:
    ...
    python genai_tools/find_and_adopt_orphans.py --dry-run

    # Actually adopt orphaned orders
    python genai_tools/find_and_adopt_orphans.py

    # Custom price tolerance
    python genai_tools/find_and_adopt_orphans.py --tolerance 1.0

Examples:
    # Preview adoptions with 0.5% price tolerance
    python genai_tools/find_and_adopt_orphans.py --dry-run --tolerance 0.5
...
from database.order import adopt_orphaned_orders
...
def display_adoption_details(result: Dict[str, Any], tolerance: float) -> None:
    ...
    if adopted or dry_run:
        ...
        for detail in details:
            print(f"  Child:      {detail.get('child_id')}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Find and adopt orphaned child orders to compatible parents",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Compatibility criteria:
  - Same product_id (e.g., BTC-USDC)
...
Examples:
  ...
  python genai_tools/find_and_adopt_orphans.py --dry-run --tolerance 1.0

  # Perform adoptions with 0.5% tolerance
  python genai_tools/find_and_adopt_orphans.py

  # Perform adoptions with 2% tolerance
  python genai_tools/find_and_adopt_orphans.py --tolerance 2.0
        """
    )
    ...
    # Validate tolerance
    if args.tolerance < 0:
        print(f"❌ Tolerance must be non-negative, got {args.tolerance}")
        return 1
    ...
    print()
    print(f"📊 Summary:")
    print(f"   Total children:        {result.get('total_children', 0)}")

...
intent reservation for an exact canonical zero-fill `REVEALED` Stealth
placement. Its market/cap terms and live execution remain explicitly unbound;
Execute is fixed-disabled, every exchange allowance remains unconsumed, and
the workflow makes zero Coinbase calls.

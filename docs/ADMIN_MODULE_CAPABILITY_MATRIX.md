...
| Futures / Perpetuals | Local sanitized/source-disabled account, position, risk, preview-artifact, command-suite, and fill-readback evidence remains available; ordinary GETs make zero Coinbase calls. | Four compatibility POST routes are registered but source-disabled and return fixed `501`/`not_implemented`; current command draft count is `0`, executable command count is `0`, and the BFF must not forward them. `POST /api/v1/futures/risk-proofs` remains a separate append-only local-evidence writer and grants no command authority. | No Futures trading dry-submit is installed; request envelopes are validated only before the fixed source-disabled boundary. | Not approved through frontend; separate source restoration and explicit authorization are required. | `/api/v1/futures/*` local/source-disabled reads, four source-disabled compatibility POSTs, and the local risk-proof evidence writer. | `position_key` for positions/reconciliation, `product_id` for placement scope, `client_order_id` for cancel scope, and `futures_risk_proof_id` for proof readback. | Never import spot-only rules. Historical service, adapter, approval, cap, audit, reconciliation, or risk-proof evidence cannot enable the source-disabled routes; no Coinbase read, exchange mutation, or command/audit/idempotency persistence occurs at the fixed boundaries. | Admin API contract tests, frontend route coverage, release gate, and contextless non-spot review. |

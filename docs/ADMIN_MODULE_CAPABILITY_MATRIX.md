...
| Campaigns / Sweeps | Spot campaign and sweep reads implemented under Spot Operations | Spot campaign execution and spot sweep automation routes accept backend-owned `dry_run=true` review requests; `dry_run=false` live execution remains disabled | Spot campaign and sweep automation dry-submit paths exist for tests, smokes, and gated no-live frontend review; UI keeps `dry_run=true`, renders backend-supplied campaign readiness and sweep control contract-check rows, and requires matched capability evidence before request | Not approved through frontend; accepted dry-runs submit 0 USDC and invoke no scheduler, runner, retry executor, reconciliation, browser/BFF execution, or Coinbase order path; campaign execution returns `campaign_execution_readiness_checks` explaining dry-run acceptance while live admission remains blocked | `/api/v1/spot/campaign/*` and `/api/v1/spot/sweep/automation-runs` today; other campaign/sweep namespaces require backend contract first | Campaign id, sweep config id, plus backend-defined order identity | Spot campaign/sweep rules are spot-only; campaign readiness rows are display-only backend evidence and are not live admission, scheduler, runner, retry, reconciliation, Coinbase, browser, or BFF authority; non-spot campaigns need separate domain contracts | Admin API regression, frontend command smoke, contextless review |

...
| Stealth Orders | Implemented as backend read-only stealth lifecycle list/detail routes and read-only command-suite readiness evidence | Implemented as live-disabled HTTP contracts for create, reveal, move, and cancel by `stealth_order_id`; recovery and reconciliation remain command-suite gaps | Implemented for tests, smokes, and gated no-live frontend review; UI may call canonical backend/BFF stealth create, reveal, move, and cancel dry-submit helpers only when capability evidence is matched, `frontend_safe=true`, and `live_enabled=false`; command-suite evidence is display-only | HTTP stealth create/reveal/move/cancel return `501` until lifecycle/trigger, mutation-claim, active-placement, exchange handling, cancel/replace, and reconciliation gates are complete; command-suite readiness does not execute create, reveal, move, reprice, recovery, reconciliation, Coinbase reads, or Coinbase calls | `/api/v1/stealth/orders`, `/api/v1/stealth/orders/{stealth_order_id}`, `/api/v1/stealth/command-suite`, `/api/v1/stealth/orders/{stealth_order_id}/reveal`, `/api/v1/stealth/orders/{stealth_order_id}/move`, `/api/v1/stealth/orders/{stealth_order_id}/cancel`; dashboard remains compatibility surface | `stealth_order_id`; active placement client id and exchange ids are evidence only | Must preserve exchange-reality state, flat hierarchy, mutation claims, and placement lifecycle rules; no spot wallet/cost-basis assumptions; no active-placement or exchange-id command keys; revealed orders cannot be locally hidden/cancelled/moved without exchange proof | Stealth regression, Admin API contract tests, frontend release gate, command dry smoke, contextless module review |

...
evidence; they grant no further Preview call, R13, Slice 3/4/5 activation, or
other live authority. R8 content and hash remain inaccessible. See
`docs/FUTURES_SLICE_2R12_PREPARATION.md` for historical preparation, not current
execution authority.
...
policy rejection that does not authorize schema or acceptance broadening. R11
grants no independent successor, Slice 3/4/5, or live authority. R12 is
governed only by its consumed terminal boundary.
...
regular=`UNSPECIFIED` plus intraday-profile=`INTRADAY`. The mapping remains
operator-defined, Preview-only, and not Coinbase-documented. R7 additionally
...
deserialized its pre-fix content. Historical R11-era default operator readback
selected exact immutable R11 and failed closed instead of falling back to R10.
Current selected terminal readback is R12 and never falls back to R11 or R10.
R8 is
terminal blocked
...
The operator review stack starts in safe No-live mode. Controlled-live is
route-specific and can admit only authenticated manual Spot LIMIT/GTC placement
and cancellation of a durable Admin manual Spot root after the full backend
authority chain passes. Cancel remains operator-keyed by `client_order_id` and
submits its authoritatively verified exchange `order_id` exactly once through
the canonical wrapper. Every other exchange mutation is No-live or
source-disabled. Futures/Perpetuals expose local sanitized/source-disabled
read evidence only. Their account, position, risk, command-suite, and
fill-readback GETs make zero Coinbase calls and expose no runtime mutation
controls.

## Historical Pre-Operator-Ready Detail

The detailed matrix below is retained as historical capability-development
evidence. Its present-tense No-live descriptions of manual Spot place/cancel
are superseded by the current route-specific authority above and must not be
used as installed runtime truth.

The Futures row's historical CFM-forbidden sentence is also superseded. The
bounded Default-profile diagnosis returned permissions, portfolio catalog,
CFM balance, and CFM positions. Installed routing now keeps Spot on
`coinbase/Test`, uses an independent `coinbase` Default-profile Futures
client, preserves the terminal Goal 10 v1 row, and exposes the zero-use
`operator_futures_manual_order_lifecycle_default_profile_v2` successor.
...
| Futures / Perpetuals | Routed account, position-list/detail, manual lifecycle, selected-position lifecycle, and Default-profile order inventory/detail readback are backend-owned. Ordinary navigation remains call-free; historical risk, Preview-artifact, command-suite, and fill evidence stays separate from current exchange truth. | The manual lifecycle supports one exact Preview-gated opening-order flow. Position lifecycle supports one exact selected-position Close or Reduce flow. Goal 2 adds explicit order-catalog refresh, exact-order reconciliation, and one independent exact Cancel for a freshly observed `OPEN` order. The four legacy generic POST routes remain source-disabled `501` compatibility boundaries and are never substitutes for these workflows. | PostgreSQL owns independent cycle budgets, payload-bound idempotency, cursor-page and mutation claims, sanitized order projections, restart recovery, fixed diagnostics, call accounting, and audit. The UI forwards only generated requests, `client_order_id`, and explicit intent; it supplies no portfolio, price, size, cap, or exchange identity. | Each workflow executes only under its exact fresh backend eligibility, Controlled-live posture, RBAC, claims, and independent per-goal allowances. Default-profile credential routing is installed; a goal-specific failed or unconsumed allowance is never transferred. | `/api/v1/futures/order-operations`, `/api/v1/futures/order-operations/{client_order_id}`, `/refresh`, exact `/reconciliation`, exact `/cancel`, `/manual-lifecycle`, `/position-lifecycle`, canonical account/position reads, technical evidence GETs, and four source-disabled legacy POST paths. | Opaque `position_key`; canonical `client_order_id`; raw exchange order identity is process-local and only its SHA-256 binding persists; credential-bound Default portfolio, candidate, eligibility, Preview, claim, and other exchange identities are hash-only. | Never import Spot wallet, Test-profile, cost-basis, no-short, profitability, or Spot-cap rules. Futures authority is Default-profile CFM account/order/position truth, margin/collateral, contracts, side, V3 policy where applicable, and exact reconciliation. Generic adapters and legacy routes cannot broaden a bounded workflow. | Goal-specific reader/service/runtime/route/PostgreSQL/restart/wrapper tests, generated-contract and authenticated operator/viewer E2E, installed Controlled-live, full regression/release-gate, safety audit, and blind-contextless audit. |

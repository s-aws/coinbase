...
| Spot Operations | Implemented for readiness, sweep status, P/L, P/L checkpoint list/detail evidence, cost basis, campaign status, direct-order audit, recovery preview, command-suite readiness with proof routes, readiness preconditions, coverage gaps, and typed coverage-gap evidence-route linkage, order reads, and frontend read-model interactions | Implemented as live-disabled HTTP contracts for manual order, cancel by `client_order_id`, campaign execution, and sweep automation keyed by `sweep_config_id`; P/L tracking has local checkpoint records with average-cost review, audit-link evidence, recovery-read linkage, and reconciliation-plan read linkage, while recovery apply/rollback, reconciliation execution/proof, and full sweep scheduler/recovery execution remain explicit command-suite coverage gaps until backend contracts exist | Implemented for tests, smokes, and gated no-live frontend review; UI may call canonical backend/BFF dry-submit helpers only when capability evidence is matched, `frontend_safe=true`, and `live_enabled=false`; P/L checkpoint records and recovery preview are backend-owned local-state/read evidence, not command drafts | HTTP command routes return `501` until live approval/cap/audit/reconciliation gates are complete; `GET /api/v1/spot/command-suite` and `GET /api/v1/spot/recovery/preview` are read-only coverage/preview evidence and never live authority; P/L checkpoint routes never submit Coinbase orders; legacy approved tools remain outside frontend authority | `/api/v1/spot/*`, `/api/v1/orders`, `/api/v1/orders/{client_order_id}` | `client_order_id`, `sweep_config_id`, `checkpoint_id`; exchange `order_id` evidence only | spot-only USDC scope, wallet inventory, no shorting, cost basis, average cost, lot authority, known profitable inventory; P/L checkpoints, average-cost review evidence, audit-link evidence, recovery-read linkage, recovery preview, and reconciliation-plan read linkage are not tax accounting, profitability proof, sell authority, browser guard evidence, recovery execution, repair apply, rollback, order/exchange-state mutation, reconciliation execution, or Coinbase calls; these rules must not become platform defaults | Admin API regression, spot readiness regression, frontend release gate, contextless spot-order review |
...
| P/L, Ledger, And Reconciliation | Implemented for spot P/L, Spot P/L checkpoint and average-cost review evidence, audit-link evidence, recovery-read linkage, recovery preview, reconciliation-plan read linkage, and fill-ledger health evidence | Implemented only as backend-owned Spot P/L checkpoint local-state records and read-only recovery preview; reconciliation execution is not modeled here | Checkpoint records are idempotent/audited backend evidence only and recovery preview is read-only; neither dry-submits Coinbase commands | Not applicable through frontend | `/api/v1/spot/sweep/pnl`, `/api/v1/spot/pnl/checkpoints`, `/api/v1/spot/pnl/checkpoints/{checkpoint_id}`, `/api/v1/spot/recovery/preview`, `/api/v1/admin/recovery-gate`, `/api/v1/admin/fill-ledger-health`, `/api/v1/admin/reconciliation/plans`, `/api/v1/admin/reconciliation/plans/{plan_id}` today | Backend-defined ledger ids, `checkpoint_id`, `audit_id`, and `client_order_id` where applicable | Spot operational P/L, average-cost review evidence, audit-link evidence, recovery-read linkage, recovery preview, and reconciliation-plan read linkage are not tax accounting; checkpoint records and recovery preview are not sell authority, profitability proof, browser guard evidence, recovery execution, repair apply, rollback, order/exchange-state mutation, reconciliation execution, reconciliation proof, or Coinbase calls; futures/perpetual P/L must be position/collateral aware | Backend read/local-state contract tests and frontend adapter tests |

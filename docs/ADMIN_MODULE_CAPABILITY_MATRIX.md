...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Admin / System Health | Implemented for bootstrap, health, session, OIDC readiness, capabilities, CSRF, live-enablement readiness, enterprise-readiness evidence, guard/risk policy, audit workbench, release gate, spot/direct-order recovery gate, fill-ledger health, and frontend fixtures | Not applicable | Not applicable | Live-enablement and enterprise-readiness routes are read-only evidence and do not enable live execution | `/api/v1/admin/*` | Request id, correlation id, actor id, backend policy ids, audit ids, and module-specific identity when reported | Platform primitive; no trading rules; live-enablement reports cap/approval/guard/audit/reconciliation requirements, controlled-live preflight check evidence, route-specific approval snapshot requirements, approval-store contract requirements, live-admission audit trail requirements, route-specific cap/guard contract requirements, live execution adapter contract evidence, and M46 normalized live readiness precondition evidence; command responses report M34 route-bound admission decision evidence; M35 persists that evidence in append-only Admin API audit events and read-only Audit Workbench rows; M36 adds backend-owned append-only approval-store infrastructure while approval snapshots remain absent and live execution remains disabled; M37 adds backend-only approval snapshot resolver infrastructure without approving commands or enabling live execution; M38 wires live-disabled command admission evidence to that resolver so snapshot-present/missing status is auditable without live authority; M39 wires live-disabled command admission evidence to backend-owned audit proof so audit-present/missing status is auditable without live authority; M40 wires live-disabled command admission evidence to backend-owned cap/guard proof so cap/guard-present/missing status is auditable without guard mutation, browser authority, or live execution; M41 wires live-disabled command admission evidence to backend-owned reconciliation plan proof so reconciliation-present/missing status is auditable without reconciliation execution, browser authority, or live execution; M42 makes the disabled backend live execution service boundary explicit on command admission evidence without adding a live switch, browser authority, or Coinbase calls; M43 adds a backend-owned disabled service descriptor with no executable create, cancel, submit, execute, browser, BFF, or Coinbase authority methods; M44 adds route-to-shared-command-service adapter evidence while keeping adapters disabled, unconfigured, and non-executable; M45 adds command admission live execution intent evidence while keeping intent not prepared, non-executable, and display-only; M46 keeps readiness preconditions derived from existing live-enablement evidence and does not create a new endpoint, command admission, route-local executor, browser approval, BFF execution authority, or Coinbase call; M47 adds enterprise-readiness `functionality_inventory` workflow rows for read, command, live, recovery, repair, automation, and legacy surfaces with exposure status and missing-contract blockers; M48 adds enterprise-readiness `mutation_taxonomy` rows that map each command route or legacy command surface to one backend-owned mutation family with identity, RBAC, idempotency, approval, cap/guard, audit, reconciliation, and service-owner evidence; enterprise-readiness also reports module support, registry ownership, unsupported actions, command gaps, and spot-rule boundaries; recovery-gate evidence is scoped to spot/direct-order recovery until non-spot module contracts exist | Admin API contract tests, frontend release gate when consumed |

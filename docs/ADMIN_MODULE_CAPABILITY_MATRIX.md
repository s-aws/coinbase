...
| P/L, Ledger, And Reconciliation | Implemented for spot P/L, Spot P/L checkpoint and average-cost review evidence, audit-link evidence, and fill-ledger health evidence | Implemented only as backend-owned Spot P/L checkpoint local-state records; reconciliation execution is not modeled here | Checkpoint records are idempotent/audited backend evidence only and do not dry-submit Coinbase commands | Not applicable through frontend | `/api/v1/spot/sweep/pnl`, `/api/v1/spot/pnl/checkpoints`, `/api/v1/spot/pnl/checkpoints/{checkpoint_id}`, `/api/v1/admin/fill-ledger-health` today | Backend-defined ledger ids, `checkpoint_id`, `audit_id`, and `client_order_id` where applicable | Spot operational P/L, average-cost review evidence, and audit-link evidence are not tax accounting; checkpoint records are not sell authority, profitability proof, browser guard evidence, recovery proof, or reconciliation proof; futures/perpetual P/L must be position/collateral aware | Backend read/local-state contract tests and frontend adapter tests |

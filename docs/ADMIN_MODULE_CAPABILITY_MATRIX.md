...
| Futures / Perpetuals | Routed account, position-list/detail, Goal 10 manual lifecycle, and Goal 11 selected-position lifecycle readback are backend-owned. Ordinary navigation remains call-free; historical risk, Preview-artifact, command-suite, and fill evidence stays separate from current exchange truth. | Goal 10 supports one exact Preview-gated Default-profile opening-order lifecycle. Goal 11 supports one exact selected-position eligibility refresh and a mutually exclusive full Close or one-contract Reduce with exact order/position reconciliation and conditional exact-order Cancel. The four legacy generic POST routes remain source-disabled `501` compatibility boundaries and are never substitutes for these workflows. | PostgreSQL owns independent cycle budgets, payload-bound idempotency, single-use claims, restart recovery, fixed diagnostics, call accounting, and audit. The operator UI may forward only generated Goal 10/11 requests and explicit intent; it supplies no portfolio, side, price, size, cap, or exchange identity. | Goal 10 and Goal 11 execute only under their exact fresh backend eligibility, dedicated Controlled-live posture, RBAC, claims, and per-goal call allowances. The installed credential currently receives a fixed HTTP-forbidden CFM-positions classification, so Goal 11 live allowances remain unconsumed. | `/api/v1/futures/manual-lifecycle`, `/manual-lifecycle/eligibility`, `/manual-lifecycle/execute`, `/position-lifecycle`, `/position-lifecycle/eligibility`, `/position-lifecycle/execute`, canonical account/position reads, technical evidence GETs, and four source-disabled legacy POST paths. | Opaque `position_key`; canonical `client_order_id`; credential-bound Default portfolio, candidate, eligibility, Preview, claim, and exchange identities are hash-only; exact revision, idempotency, correlation, and audit identities remain backend-owned. | Never import Spot wallet, Test-profile, cost-basis, no-short, profitability, or Spot-cap rules. Futures authority is Default-profile CFM position, margin/collateral, contracts, side, V3 policy where applicable, and exact reconciliation. Generic adapters and legacy routes cannot broaden either bounded workflow. | Goal 10/11 service, runtime, route, PostgreSQL/restart, wrapper, generated-contract, operator/viewer E2E, installed Controlled-live, full regression/release-gate, safety-audit, and blind-contextless tests. |

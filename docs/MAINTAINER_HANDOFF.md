...
- Active autonomous range: `1681-1700`.
- M49 approval lifecycle, M50 cap/guard records, M51 admission audits, and
  ...
  routes or live controls exist. The active M54 range links those gap rows to
  typed `current_read_evidence` rows derived from route inventory so the
  website can navigate to existing read-only evidence without adding command
  workflow drafts, BFF mutation routes, or live controls. Live Coinbase

...
- Active autonomous range: `1981-2000`.
- M49 approval lifecycle, M50 cap/guard records, M51 admission audits, and
  ...
  or prove live exchange truth. The active 1981-2000 range starts M55 by
  adding read-only stealth command-suite readiness evidence for create,
  cancel, reveal, move, reprice, recovery, and reconciliation workflows. It
  may expose existing live-disabled stealth cancel and movement/reprice route
  linkage plus missing contracts and exchange-truth blockers, but it must not
  create stealth orders, reveal orders, cancel active placements, move/reprice
  revealed orders, mutate stealth/order/exchange state, execute
  reconciliation, read Coinbase, or grant browser/BFF stealth command
  authority. This foundation must not add a parallel writer,
  ...
  The active 1981-2000 range must define backend-owned stealth command-suite
  readiness as no-live evidence before any stealth create/reveal/cancel/move/

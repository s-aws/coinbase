...
| `POST /api/v1/spot/recovery/apply-executions` | `local_state_mutation` | `spot_recovery:execute` | required | required | required | required | `execute_spot_recovery_apply` | spot recovery apply execution remains fail-closed; no repair apply, order/exchange-state mutation, Coinbase read, or Coinbase REST placement | |
| `POST /api/v1/spot/recovery/rollback-executions` | `local_state_mutation` | `spot_recovery:execute` | required | required | required | required | `execute_spot_recovery_rollback` | spot recovery rollback execution remains fail-closed; no rollback mutation, order/exchange-state mutation, Coinbase read, or Coinbase REST placement | |

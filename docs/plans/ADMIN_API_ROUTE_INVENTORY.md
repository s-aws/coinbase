...
| `GET /api/v1/admin/enterprise-readiness` | `read_only` | `analytics:read` | not required | not required | read-only release evidence only | optional read audit | `build_enterprise_readiness` | read-only M9/M20 module support, unsupported action, structured command-gap, security, and release evidence | |

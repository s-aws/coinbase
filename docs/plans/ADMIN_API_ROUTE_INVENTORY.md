...
| `POST /api/v1/spot/recovery/exchange-state-proofs` | `local_state_mutation` | `spot_recovery:execute` | required | required | required | required | `record_spot_recovery_exchange_state_proof` | spot recovery exchange-state proof writing remains fail-closed; no proof persistence, order/exchange-state mutation, Coinbase read, or Coinbase REST placement | |
| `POST /api/v1/spot/recovery/reconciliation-proofs` | `local_state_mutation` | `spot_recovery:execute` | required | required | required | required | `record_spot_recovery_reconciliation_proof` | spot recovery reconciliation proof writing remains fail-closed; no proof persistence, reconciliation execution, order/exchange-state mutation, Coinbase read, or Coinbase REST placement | |

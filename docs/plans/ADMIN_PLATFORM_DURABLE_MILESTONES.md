...
| M58 - Automation, Campaign, Scheduler, And Retry Suite | Operator controls completed through 8081-8100; executor work remains gated | Complete durable scheduling, run limits, pause/resume, retries, operator status, and recovery for automation without browser schedulers or parallel live paths. |
...
| M61 - Release 0.1 Private Operator Admin MVP | In Progress; active range 8101-8120 | Release a usable private operator admin product that can manage backend-supported workflows while explicit gaps appear as `unsupported` or `not_modeled`. |

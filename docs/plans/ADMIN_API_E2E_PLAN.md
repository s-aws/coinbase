...
backend-owned readiness closure plan derived from existing readiness
decisions, prerequisites, request fields, semantic guards, evidence routes,
and missing backend contracts. The work must remain read-only and no-live: no
futures command route, command draft, manager invocation, exchange order

...
and direct-order audit readback with `dashboard_dependency=false`. The next
blocker is Spot SELL authority: Admin API lot-authority and planned-budget
sources must exist before SELL is operator-completable. Cancel should stay out
of the operator-completable slice until it has an explicit acknowledgement
contract.

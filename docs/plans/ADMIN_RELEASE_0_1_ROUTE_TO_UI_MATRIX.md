...
| Automation/campaigns | Spot campaign status, campaign executions, sweep status, sweep P/L, sweep automation runs, `GET /api/v1/spot/command-suite` `automation_control_readiness` | Campaigns, Spot Operations, Command Workflows | `blocked` | Campaign and sweep `dry_run=true` review is operator-visible through accepted backend responses, with scheduler/runner/Coinbase flags false and `submitted_notional_usdc=0` / `executed_notional_usdc=0`. The command suite now exposes six typed backend-owned readiness rows for scheduler, run limit, pause/resume, retry/recovery, reconciliation execution, and live execution; all remain `not_modeled` / blocked with browser/BFF display-only boundaries and zero notional. `dry_run=false` live execution remains gated. | Next implementation must create an exact backend-owned automation service contract or closeout runbook for these blockers; do not add browser scheduler, BFF runner authority, or a second sweep execution path. |

...
| Automation/campaigns | Spot campaign status, campaign executions, sweep status, sweep P/L, sweep automation runs | Campaigns, Spot Operations, Command Workflows | `blocked` | Campaign and sweep `dry_run=true` review is operator-visible through accepted backend responses, with scheduler/runner/Coinbase flags false and `submitted_notional_usdc=0` / `executed_notional_usdc=0`. Scheduler/run-limit/retry/pause-resume behavior remains not operator-complete, and `dry_run=false` live execution remains gated. | Inventory campaign/sweep scheduler state and represent missing controls as `not_modeled`; do not add browser scheduler or runner authority. |

...
The next implementation slice is active `8041-8060` Spot Command Operator
E2E. Inventory read coverage is now `ready_with_data_gate`; the remaining
Release 0.1 spot blocker is no longer route-local command flag binding,
generic live-service dependency wiring, BUY post-submit audit handoff, SELL
operator-facing validation evidence, or movement/repricing action-state
visibility. Manual order and cancel route adapters now source
`allow_live_execution` from backend live admission, and manual order has a
route-scoped configured backend live-service dependency. Manual order
admission can pass with exact backend records, manual acknowledgement, and a
completed backend live-service state. Accepted configured live responses now
...
frontend command workflow renders and links that backend evidence without
creating browser/BFF execution authority, and the Spot Operations panel can

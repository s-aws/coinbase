...
remaining blocker is making one manual Spot BUY path operator-completable
through frontend controls that display/use that backend evidence, live
Coinbase validation under the approved notional cap, and eventual
reconciliation execution proof. SELL must wait for Admin API lot-authority and
planned-budget sources; cancel should stay out of the operator-completable
slice until it has an explicit acknowledgement contract.

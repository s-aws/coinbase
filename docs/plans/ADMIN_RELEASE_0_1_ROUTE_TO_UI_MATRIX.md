...
| Automation/campaigns | Spot campaign status, campaign executions, sweep status, sweep P/L, sweep automation service status, sweep automation runs, `GET /api/v1/spot/command-suite` `automation_control_readiness` | Campaigns, Spot Operations, Command Workflows | `blocked` | Campaign and sweep `dry_run=true` review is operator-visible through accepted backend responses, with scheduler/runner/Coinbase flags false and `submitted_notional_usdc=0` / `executed_notional_usdc=0`. `GET /api/v1/spot/sweep/automation-service` now exposes backend-owned read-only campaign ledger, sweep ledger, operation-lock, scheduler-status, retry-plan, missing-contract, and no-live proof evidence for the Campaigns UI. The command suite exposes typed backend-owned readiness rows for scheduler, run limit, pause/resume, retry/recovery, reconciliation execution, and live execution; incomplete controls remain `not_modeled` / blocked with browser/BFF display-only boundaries and zero notional. `dry_run=false` live execution remains gated. | Next implementation must create the exact backend-owned pause/resume and retry control contract, or explicitly close that blocker in the operator runbook; do not add browser scheduler, BFF runner authority, or a second sweep execution path. |

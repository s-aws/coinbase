...
- The canonical regression runner uses short tracebacks and a Windows memory-pressure guard. It samples every 5 seconds and aborts on high commit pressure, high physical-memory pressure, or low available physical memory. A `memory_guard_aborted` summary is a failed closeout gate; run the stale-process checker, preserve evidence, and split or reduce the offending regression surface before retrying. Do not use `--disable-memory-watch` for normal closeout.

...
- The canonical regression runner uses short tracebacks and a Windows memory-pressure guard. A `memory_guard_aborted` summary is a failed closeout gate; run the stale-process checker, preserve evidence, and split or reduce the offending regression surface before retrying. Do not use `--disable-memory-watch` for normal closeout.
